...
class PortsConnectivityStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, "Cổng & Kết nối", "Các cổng kết nối, Wi-Fi, Bluetooth là những thành phần thiết yếu. Một cổng hỏng sẽ gây nhiều phiền toái.", "**Tự động:** Trạng thái Wi-Fi được cập nhật bên dưới.\n**Thủ công:** Lần lượt cắm thiết bị vào các cổng (LAN, USB, HDMI, Audio, Thẻ SD) và đánh dấu vào checkbox tương ứng.\n**Nâng cao:** Nhấn 'Quét Mạng Wi-Fi' để kiểm tra khả năng thu sóng của card mạng.", **kwargs)
        ...
        self.wifi_status_label = ctk.CTkLabel(status_container, text="Wi-Fi: Đang kiểm tra...", font=Theme.BODY_FONT, justify="left"); self.wifi_status_label.pack(pady=(0, 20), padx=20, anchor="w")
    ...
    def scan_wifi_networks(self):
        ...
        def do_scan():
            ...
            try:
                ...
                if not ifaces: raise Exception("Không tìm thấy card Wi-Fi.")
                iface = ifaces[0]; iface.scan(); time.sleep(5)
                networks = sorted(iface.scan_results(), key=lambda x: x.signal, reverse=True)
            except Exception as e: error_msg = f"Lỗi khi quét Wi-Fi:\n{e}"
            
            # Safely update UI on main thread
            def update_ui():
                if not scan_window.winfo_exists(): return
                status_label.pack_forget()
                results_frame.pack(expand=True, fill="both", padx=10, pady=10)
                if error_msg: ctk.CTkLabel(results_frame, text=error_msg, text_color=Theme.ERROR).pack(); return
                if not networks: ctk.CTkLabel(results_frame, text="Không tìm thấy mạng Wi-Fi nào.").pack(); return
                
                for res in networks:
                    net_frame = ctk.CTkFrame(results_frame, fg_color=Theme.FRAME)
            ...
            if scan_window.winfo_exists(): scan_window.after(0, update_ui)
        ...
        try:
            ...
            for name, stat_data in stats.items():
                if "wi-fi" in name.lower() or "wlan" in name.lower():
                    wifi_status = "Đang kết nối" if stat_data.isup else "Đã tắt hoặc lỗi Driver"
...
class SystemStabilityStep(BaseStepFrame):
    ...
    def _scan_thread(self):
        try:
            ...
            if self.winfo_exists(): self.after(0, self.display_results, events)
...
class WizardFrame(ctk.CTkFrame):
    ...
    def go_to_step(self, step_index):
        ...
        if self.current_frame is not None:
            ...
            if isinstance(self.current_frame, SummaryStep): self.current_frame.grid_forget()
            else: self.current_frame.pack_forget()
        ...
        if isinstance(self.current_frame, SummaryStep):
            ...
            if any(isinstance(s, SummaryStep) for s in self.steps_cache.values()):
