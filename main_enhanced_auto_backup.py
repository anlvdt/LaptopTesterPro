...
class PhysicalInspectionStep(BaseStepFrame):
    ...
    def create_inspection_checklist(self):
        checklist_frame = ctk.CTkFrame(self.action_frame, fg_color=Theme.FRAME, corner_radius=Theme.CORNER_RADIUS)
        checklist_frame.grid(row=0, column=0, sticky="ew", padx=20, pady=10)
        ctk.CTkLabel(checklist_frame, text="🔍 Checklist Kiểm Tra Ngoại Hình Chi Tiết", font=Theme.SUBHEADING_FONT, text_color=Theme.ACCENT).pack(pady=15)
        
        # Vỏ máy & Bản lề
        section1 = ctk.CTkFrame(checklist_frame, fg_color=Theme.BACKGROUND)
        section1.pack(fill="x", padx=15, pady=5)
        ctk.CTkLabel(section1, text="💻 Vỏ Máy & Bản Lề:", font=Theme.BODY_FONT, text_color=Theme.ACCENT).pack(anchor="w", padx=10, pady=5)
        for item in [
        ...
        ctk.CTkLabel(section2, text="🔌 Cổng Kết Nối:", font=Theme.BODY_FONT, text_color=Theme.ACCENT).pack(anchor="w", padx=10, pady=5)
        for item in [
        ...
        ctk.CTkLabel(section3, text="🔩 Ốc Vít & Tem Bảo Hành:", font=Theme.BODY_FONT, text_color=Theme.ACCENT).pack(anchor="w", padx=10, pady=5)
        for item in [
        ...
        ctk.CTkLabel(thinkpad_frame, text="🔴 LENOVO THINKPAD - KIỂM TRA ĐẶC BIỆT:", font=Theme.BODY_FONT, text_color="#60A5FA").pack(anchor="w", padx=10, pady=5)
        for item in [
    ...
    def show_result_choices(self):
        for widget in self.result_container.winfo_children():
            widget.destroy()
        question_text = "Tình trạng vật lý tổng thể của máy như thế nào?" if CURRENT_LANG == "vi" else "What is the overall physical condition of the laptop?"
        ctk.CTkLabel(self.result_container, text=question_text, font=Theme.SUBHEADING_FONT).pack(pady=15)
        button_bar = ctk.CTkFrame(self.result_container, fg_color="transparent")
        button_bar.pack(pady=15)
        
        btn1_text = "Rất tốt" if CURRENT_LANG == "vi" else "Excellent"
        btn2_text = "Tốt" if CURRENT_LANG == "vi" else "Good"
        btn3_text = "Trung bình" if CURRENT_LANG == "vi" else "Average"
        btn4_text = "Kém" if CURRENT_LANG == "vi" else "Poor"
...
class BIOSCheckStep(BaseStepFrame):
    ...
    def create_bios_checklist(self):
        checklist_frame = ctk.CTkFrame(self.action_frame, fg_color=Theme.FRAME, corner_radius=Theme.CORNER_RADIUS)
        checklist_frame.grid(row=0, column=0, sticky="ew", padx=20, pady=10)
        ctk.CTkLabel(checklist_frame, text="⚙️ Checklist Kiểm Tra BIOS Chi Tiết", font=Theme.SUBHEADING_FONT, text_color=Theme.ACCENT).pack(pady=15)
        
        # Cách vào BIOS
        access_frame = ctk.CTkFrame(checklist_frame, fg_color=Theme.BACKGROUND)
        access_frame.pack(fill="x", padx=15, pady=5)
        ctk.CTkLabel(access_frame, text="🔑 Cách Vào BIOS:", font=Theme.BODY_FONT, text_color=Theme.ACCENT).pack(anchor="w", padx=10, pady=5)
        for item in [
        ...
        ctk.CTkLabel(cpu_frame, text="⚡ Hiệu Năng CPU:", font=Theme.BODY_FONT, text_color=Theme.ACCENT).pack(anchor="w", padx=10, pady=5)
        for item in [
        ...
        ctk.CTkLabel(password_warning, text="⛔ CẢNH BÁO NGHIÊM TRỌNG - MẬT KHẨU BIOS", font=Theme.SUBHEADING_FONT, text_color="#FEE2E2").pack(pady=10)
        ...
        ctk.CTkLabel(security_frame, text="🔒 BẢO MẬT KHÁC:", font=Theme.BODY_FONT, text_color="#FCA5A5").pack(anchor="w", padx=10, pady=5)
        for item in[
        ...
        thinkpad_frame = ctk.CTkFrame(checklist_frame, fg_color="#1E3A8A", border_width=2, border_color="#3B82F6")
        thinkpad_frame.pack(fill="x", padx=15, pady=10)
        ctk.CTkLabel(thinkpad_frame, text="🔴 THINKPAD - CÀI ĐẶT KHÁC:", font=Theme.BODY_FONT, text_color="#60A5FA").pack(anchor="w", padx=10, pady=5)
        for item in [
    ...
    def show_result_choices(self):
        for widget in self.result_container.winfo_children():
            widget.destroy()
        question_text = "Các cài đặt trong BIOS có chính xác và an toàn không?" if CURRENT_LANG == "vi" else "Are the BIOS settings correct and safe?"
        ctk.CTkLabel(self.result_container, text=question_text, font=Theme.SUBHEADING_FONT).pack(pady=15)
        button_bar = ctk.CTkFrame(self.result_container, fg_color="transparent")
        button_bar.pack(pady=15)
        
        btn_yes_text = "Có, mọi cài đặt đều đúng" if CURRENT_LANG == "vi" else "Yes, all settings are correct"
        btn_no_text = "Không, có cài đặt sai/bị khóa" if CURRENT_LANG == "vi" else "No, incorrect settings/locked"
    ...
    def get_battery_info(self):
        ...
        try:
            ...
            if battery:
                ...
                ctk.CTkLabel(rec_frame, text="💡 LỜI KHUYÊN VỀ SỨC KHỎE PIN", font=Theme.SUBHEADING_FONT, text_color=Theme.ACCENT).pack(pady=(15,10))
                
                # Status-specific recommendations
                if health_percent < 60:
                    status_frame = ctk.CTkFrame(rec_frame, fg_color=Theme.ERROR)
                    status_frame.pack(fill="x", padx=15, pady=5)
                    ctk.CTkLabel(status_frame, text="⚠️ TÌNH TRẠNG: Pin suy giảm nghiêm trọng", font=Theme.BODY_FONT, text_color="white").pack(pady=5)
                    status_recs = [
                    ...
                    status_frame.pack(fill="x", padx=15, pady=5)
                    ctk.CTkLabel(status_frame, text="⚠️ TÌNH TRẠNG: Pin bắt đầu suy giảm", font=Theme.BODY_FONT, text_color="white").pack(pady=5)
                    status_recs = [
                    ...
                    status_frame.pack(fill="x", padx=15, pady=5)
                    ctk.CTkLabel(status_frame, text="✅ TÌNH TRẠNG: Pin còn tốt", font=Theme.BODY_FONT, text_color="white").pack(pady=5)
                    status_recs = [
...
class SummaryStep(BaseStepFrame):
    ...
    def create_simple_summary(self, results):
        ...
        if capabilities:
            ...
            ctk.CTkLabel(cap_header, text="💡 Khả Năng Sử Dụng Phần Cứng", font=("Arial", 28, "bold"), text_color=Theme.ACCENT).pack(anchor="w")
        ...
        # AI Disclaimer
        disclaimer_text = "⚠️ LƯU Ý: Ứng dụng được phát triển dựa trên AI nên có thể có sai sót. Khuyến khích kiểm tra thêm bằng các ứng dụng chuyên dụng được cung cấp bên dưới." if CURRENT_LANG == "vi" else "⚠️ DISCLAIMER: This AI-developed application may contain errors. We recommend additional verification using the professional tools provided below."
        ...
        tools_desc_text = "Để xác minh kết quả AI và kiểm tra sâu hơn, hãy sử dụng các công cụ chuyên nghiệp sau:" if CURRENT_LANG == "vi" else "To verify AI results and perform deeper inspection, use these professional tools:"
...
class App(ctk.CTk):
    def __init__(self):
        ...
        address_text = "📍 237/1C Tôn Thất Thuyết, P. Vĩnh Hội, (P.3, Q.4 cũ), TPHCM" if CURRENT_LANG == "vi" else "📍 237/1C Ton That Thuyet St., Vinh Hoi Ward, (Old Ward 3, Dist. 4), HCMC"
