...
LANG = {
    "vi": {
        # Main UI
        "title": t("LaptopTester Pro - Kiểm tra laptop toàn diện"),
        ...
        "thermal_monitor": "Giám sát nhiệt độ", "system_stability": "Ổn định hệ thống",
        "physical_inspection": "Kiểm Tra Ngoại Hình", "bios_check": "Kiểm Tra BIOS",
        ...
        "test_completed": "Test hoàn thành",
        "user_proceeded": "Người dùng đã chuyển sang bước tiếp theo",
        "all_tests_completed": "Tất cả các bài kiểm tra đã hoàn thành!",
        "final_summary": "TỔNG KẾT CUỐI CÙNG",
        ...
        # Time related
        "Thời gian:": "Thời gian:", "Tiến độ:": "Tiến độ:",
        "Test đã dừng bởi người dùng (ESC)": "Test đã dừng bởi người dùng (ESC)",
        "Test đã dừng bởi người dùng": "Test đã dừng bởi người dùng",
        "Nhấn ESC để dừng": "Nhấn ESC để dừng",
        "GPU Stress Test - Nhấn ESC để thoát": "GPU Stress Test - Nhấn ESC để thoát",
        "Đang khởi tạo Pygame...": "Đang khởi tạo Pygame...",
        "Đang chạy vòng lặp stress...": "Đang chạy vòng lặp stress...",
        "Pygame không có sẵn. Cài đặt: pip install pygame": "Pygame không có sẵn. Cài đặt: pip install pygame",
        "Test tổng hợp CPU+GPU+RAM để đảm bảo hệ thống ổn định dưới tải nặng kéo dài.": "Test tổng hợp CPU+GPU+RAM để đảm bảo hệ thống ổn định dưới tải nặng kéo dài.",
        "Nhấn 'Bắt đầu Test' để chạy test kết hợp 3-5 phút. Quan sát nhiệt độ và hiệu năng.": "Nhấn 'Bắt đầu Test' để chạy test kết hợp 3-5 phút. Quan sát nhiệt độ và hiệu năng.",
        "✅ Test hoàn thành": "✅ Test hoàn thành",
        "✓ Hệ thống ổn định": "✓ Hệ thống ổn định",
        "✗ Có vấn đề": "✗ Có vấn đề",
        "📊 Kết Quả GPU Stress Test": "📊 Kết Quả GPU Stress Test",
    ...
    "en": {
        ...
        "thermal_monitor": "Thermal Monitor", "system_stability": "System Stability",
        "physical_inspection": "Physical Inspection", "bios_check": "BIOS Check",
        ...
        "user_proceeded": "User proceeded to the next step",
        "all_tests_completed": "All tests have been completed!",
        ...
        # Time related
        "Thời gian:": "Time:", "Tiến độ:": "Progress:",
        ...
        "Nhấn 'Bắt đầu Test' để chạy test kết hợp 3-5 phút. Quan sát nhiệt độ và hiệu năng.": "Click 'Start Test' to run combined test for 3-5 minutes. Monitor temperature and performance.",
        "✅ Test hoàn thành": "✅ Test completed",
        "✓ Hệ thống ổn định": "✓ System stable",
        "✗ Có vấn đề": "✗ Issues found",
    ...
    try:
        ...
        with open(test_file_path, "wb") as f:
            for i in range(chunks):
                ...
                operation_text = "Ghi" if CURRENT_LANG == "vi" else "Write"
        ...
        with open(test_file_path, "rb") as f:
            for i in range(chunks):
                ...
                operation_text = "Đọc" if CURRENT_LANG == "vi" else "Read"
            queue.put({'type': 'update', 'progress': progress, 'operation': operation_text, 'speed': chunk_speed, 'avg_speed': avg_speed})
...
def run_gpu_stress(duration, queue):
    ...
    try:
        ...
        pygame.init()
        # NOT fullscreen - windowed mode so user can access Stop button
        ...
        while running and (time.time() - start_time < duration):
            ...
            # Draw performance info with translations
            ...
            if int(current_time * 2) % 2 == 0:  # Blink every 0.5 seconds
                ...
                esc_text = font_small.render(esc_hint, True, (255, 255, 0))
...
class BaseStepFrame(ctk.CTkFrame):
    def __init__(self, master, title, why_text, how_text, **kwargs):
        ...
        self.title = title
        self.step_key = kwargs.get("step_key", title)  # Use fixed key for results storage
    ...
    def setup_layout(self, why_text, how_text):
        ...
        if not self.hide_why_section:
            ...
            # Main action area container with scroll buttons
            action_outer = ctk.CTkFrame(self, fg_color="transparent")
            action_outer.grid(row=0, column=1, sticky="nsew", padx=(Theme.SPACING, Theme.PADDING), pady=Theme.PADDING)
            action_outer.grid_columnconfigure(0, weight=1)
            action_outer.grid_rowconfigure(0, weight=1)
            
            # Main action area - scrollable
            self.action_frame = ctk.CTkScrollableFrame(action_outer, fg_color=Theme.CARD, corner_radius=Theme.CORNER_RADIUS, border_width=1, border_color=Theme.BORDER)
            self.action_frame.grid(row=0, column=0, sticky="nsew")
            
            # Add scroll buttons to the right side
            self._create_scroll_buttons(action_outer)
        else:
            # Full width when hiding guide panel
            action_outer = ctk.CTkFrame(self, fg_color="transparent")
            action_outer.grid(row=0, column=0, sticky="nsew", padx=Theme.PADDING, pady=Theme.PADDING)
            action_outer.grid_columnconfigure(0, weight=1)
            action_outer.grid_rowconfigure(0, weight=1)
    ...
    def _create_scroll_buttons(self, parent):
        """Create scroll up/down buttons for the action frame"""
        ...
        def scroll_to_top():
            try:
                ...
                if hasattr(self.action_frame, '_canvas') and self.action_frame._canvas:
                    self.action_frame._canvas.yview_moveto(0)
                    self.action_frame._canvas.update_idletasks()
                elif hasattr(self.action_frame, '_parent_canvas') and self.action_frame._parent_canvas:
                    self.action_frame._parent_canvas.yview_moveto(0)
                    self.action_frame._parent_canvas.update_idletasks()
                else:
                    # Use after to ensure proper timing
                    self.action_frame.after(10, lambda: self.action_frame._canvas.yview_moveto(0) if hasattr(self.action_frame, '_canvas') else None)
            except Exception as e:
                print(f"[DEBUG] Scroll top error: {e}")
        
        def scroll_to_bottom():
            try:
                # Try multiple ways to scroll
                if hasattr(self.action_frame, '_canvas') and self.action_frame._canvas:
                    self.action_frame._canvas.yview_moveto(1)
                    self.action_frame._canvas.update_idletasks()
                elif hasattr(self.action_frame, '_parent_canvas') and self.action_frame._parent_canvas:
                    self.action_frame._parent_canvas.yview_moveto(1)
                    self.action_frame._parent_canvas.update_idletasks()
                else:
                    # Use after to ensure proper timing
                    self.action_frame.after(10, lambda: self.action_frame._canvas.yview_moveto(1) if hasattr(self.action_frame, '_canvas') else None)
            except Exception as e:
                print(f"[DEBUG] Scroll bottom error: {e}")
        ...
        down_btn.pack(side="bottom", pady=2)
    ...
    def mark_completed(self, result_data, auto_advance=False):
        ...
        if self.record_result:
            # Use step_key instead of step_name for consistent storage
            key_to_use = getattr(self, 'step_key', self.title)
            self.record_result(key_to_use, result_data)
        def scroll_to_bottom():
            try:
...
class HardwareFingerprintStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        kwargs["step_key"] = "hardware_fingerprint"  # Fixed key
    ...
    def fetch_hardware_info(self):
        ...
        if platform.system() == "Windows" and wmi and pythoncom:
            ...
            try:
                ...
                model_key = "Model Laptop" if CURRENT_LANG == "vi" else "Laptop Model"
                hw_info[model_key] = f"{system_info.Manufacturer} {system_info.Model}"
                
                # BIOS info
                bios = c.Win32_BIOS()[0]
                serial_key = "Serial Number"
                hw_info[serial_key] = getattr(bios, 'SerialNumber', 'N/A')
                ...
                try:
                    ...
                    hdd_key = "Ổ cứng" if CURRENT_LANG == "vi" else "Hard Drive"
    ...
    def _analyze_capabilities(self, cpu_name, gpu_name, ram_gb=0):
        ...
        if cpu_tier in ["high", "mid-high"] and gpu_tier in ["high", "mid-high"] and ram_tier == "high":
            capabilities = [
                ...
                {"icon": "💼", "title": "Workstation Pro", "desc": t("Code: VS, Android Studio, Docker\nVM: 3-4 máy ảo\n50+ Chrome tabs"), "color": "#3B82F6"}
            ]
    ...
    def show_result_choices(self):
        ...
        confirm_text = t("Định danh phần cứng đã hoàn thành. Phần cứng có khớp với thông tin quảng cáo?")
        ctk.CTkLabel(self.result_container, text=confirm_text, font=Theme.SUBHEADING_FONT, wraplength=900).pack(pady=15)
        
        button_bar = ctk.CTkFrame(self.result_container, fg_color="transparent")
        button_bar.pack(pady=15)
        
        self.btn_yes = ctk.CTkButton(button_bar, text=f"✓ {get_text('all_good')}", command=lambda: self.handle_result_generic(True, {"Kết quả": t("Phần cứng khớp"), "Trạng thái": "Tốt"}, {}), fg_color=Theme.SUCCESS, hover_color="#1a7f37", height=32, font=Theme.BUTTON_FONT, corner_radius=6, border_width=0)
        self.btn_yes.pack(side="left", padx=Theme.SPACING)
        
        self.btn_no = ctk.CTkButton(button_bar, text=f"✗ {get_text('issues_found')}", command=lambda: self.handle_result_generic(False, {}, {"Kết quả": t("Phần cứng không khớp"), "Trạng thái": "Lỗi"}), fg_color=Theme.ERROR, hover_color="#cf222e", height=32, font=Theme.BUTTON_FONT, corner_radius=6, border_width=0)
...
class LicenseCheckStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        kwargs["step_key"] = "license_check"  # Fixed key
    ...
    def show_result_choices(self):
        ...
        # Get the result data from mark_completed
        result_data = self.result_data.copy() if hasattr(self, 'result_data') else {}
        license_status = result_data.get("Trạng thái", "")
        
        confirm_text = t("Kiểm tra bản quyền hoàn tất. Bạn có chấp nhận kết quả này?")
        ctk.CTkLabel(self.result_container, text=confirm_text, font=Theme.SUBHEADING_FONT, wraplength=900).pack(pady=15)
        
        button_bar = ctk.CTkFrame(self.result_container, fg_color="transparent", bg_color="transparent")
        button_bar.pack(pady=15)
        
        # "All Good" keeps the test result (use default parameter to capture by value)
        self.btn_yes = ctk.CTkButton(button_bar, text=f"✓ {get_text('all_good')}", command=lambda rd=result_data: self.handle_result_generic(True, rd, {}), fg_color=Theme.SUCCESS, hover_color="#1a7f37", height=32, font=Theme.BUTTON_FONT, corner_radius=6, border_width=0)
        self.btn_yes.pack(side="left", padx=Theme.SPACING)
        
        # "Issues Found" overrides with error status
        self.btn_no = ctk.CTkButton(button_bar, text=f"✗ {get_text('issues_found')}", command=lambda: self.handle_result_generic(False, {}, {"Kết quả": t("Bản quyền có vấn đề"), "Trạng thái": "Lỗi"}), fg_color=Theme.ERROR, hover_color="#cf222e", height=32, font=Theme.BUTTON_FONT, corner_radius=6, border_width=0)
        self.btn_no.pack(side="left", padx=Theme.SPACING)
...
class SystemInfoStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        self.info_labels = {}
        self.info_items = ["CPU", "RAM", "GPU"]
        if CURRENT_LANG == "vi":
            self.info_items.append("Ổ cứng")
        else:
            self.info_items.append("Hard Drive")
        ...
        try:
            ...
            if platform.system() == "Windows" and wmi and pythoncom:
                ...
                try:
                    ...
                    disk_key = "Ổ cứng" if CURRENT_LANG == "vi" else "Hard Drive"
                    full_info[disk_key] = "; ".join(disk_details) if disk_details else not_found_text
                except Exception as e:
                    wmi_error_text = f"Lỗi WMI: {e}" if CURRENT_LANG == "vi" else f"WMI Error: {e}"
                ...
                windows_only_text = t("Chỉ hỗ trợ Windows")
                disk_key = "Ổ cứng" if CURRENT_LANG == "vi" else "Hard Drive"
                full_info.update({k: windows_only_text for k in ["CPU", "GPU", disk_key]})
        except Exception as e:
...
class HardDriveHealthStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        kwargs["step_key"] = "harddrive_health"
...
class ScreenTestStep(BaseStepFrame):
    ...
    def create_screen_test(self):
        ...
        test_title = "Automatic Display Test" if CURRENT_LANG == "en" else "Test Màn Hình Tự Động"
        ctk.CTkLabel(test_frame, text=test_title, font=Theme.SUBHEADING_FONT, text_color=Theme.ACCENT).pack(pady=15)
        ...
        self.screen_start_btn = ctk.CTkButton(btn_frame, text=get_text('start_test_btn'), command=self.start_screen_test, fg_color=Theme.ACCENT, height=Theme.BUTTON_HEIGHT, font=Theme.BODY_FONT)
        self.screen_start_btn.pack(side="left", padx=5)
        self.screen_stop_btn = ctk.CTkButton(btn_frame, text=get_text('stop_test_btn'), command=self.stop_screen_test, fg_color=Theme.ERROR, height=Theme.BUTTON_HEIGHT, font=Theme.BODY_FONT, state="disabled")
        self.screen_stop_btn.pack(side="left", padx=5)
    ...
    def start_screen_test(self):
        ...
        self.screen_start_btn.configure(state="disabled")
        ...
        def run_test():
            ...
            for i, ((bg, fg), name) in enumerate(zip(colors, names)):
                ...
                esc_text = "ESC/Stop to exit" if CURRENT_LANG == "en" else "ESC/Stop để dừng"
            ...
            if self.test_running:
                stop_test()
            self.after(0, lambda: self.screen_start_btn.configure(state="normal"))
            self.after(0, lambda: self.screen_stop_btn.configure(state="disabled"))
...
class KeyboardTestStep(BaseStepFrame):
    ...
    def create_touchpad_test(self):
        ...
        touchpad_title = "Touchpad & Mouse Test:" if CURRENT_LANG == "en" else "Test Touchpad & Chuột:"
        ctk.CTkLabel(touchpad_frame, text=touchpad_title, font=Theme.SUBHEADING_FONT, text_color=Theme.ACCENT).pack(pady=10)
    ...
    def start_listening(self):
        try:
            ...
            # suppress=False để không chặn ESC và các phím khác
...
class CPUStressTestStep(BaseStressTestStep):
    def __init__(self, master, **kwargs):
        ...
        if CURRENT_LANG == "vi":
            why_text = "Một CPU quá nhiệt sẽ tự giảm hiệu năng (throttling) gây giật lag. Bài test này sẽ đẩy CPU lên 100% tải để kiểm tra khả năng tản nhiệt của máy."
            how_text = "Nhấn 'Bắt đầu Test' để chạy test 2 phút. Theo dõi biểu đồ nhiệt độ và tần số. Nếu nhiệt độ ổn định dưới 95°C và không có throttling nghiêm trọng, hệ thống tản nhiệt hoạt động tốt. Có thể nhấn 'Dừng Test' bất cứ lúc nào."
        else:
            ...
            how_text = "Click 'Start Test' to run 2-minute test. Monitor temperature and frequency charts. If temperature stays below 95°C without severe throttling, cooling system works well. Can click 'Stop Test' anytime."
        super().__init__(master, title, why_text, how_text, **kwargs)
        self.TEST_DURATION = 120
        self.estimated_time = "2-3 phút" if CURRENT_LANG == "vi" else "2-3 mins"
    ...
    def finalize_test(self, msg):
        ...
        if max_freq and min_freq:
            ...
            if freq_locked:
                freq_ratio = min_freq / max_freq
                if min_freq < 1000:  # <1GHz
                    lock_severity = "CRITICAL"
                elif freq_ratio < 0.5:  # <50% tần số tối đa
                    lock_severity = "WARNING"
                elif freq_ratio < 0.7:  # 50-70% tần số tối đa
                    lock_severity = "MODERATE"
                else:  # >=70% tần số tối đa
                    lock_severity = "ACCEPTABLE"
        
        # Frequency lock analysis
        if freq_locked:
            # Chọn màu theo mức độ nghiêm trọng
            if lock_severity == "CRITICAL":
                lock_color = Theme.ERROR
            elif lock_severity == "WARNING":
                lock_color = "#DC2626"  # Red-600
            elif lock_severity == "MODERATE":
                lock_color = Theme.WARNING
            else:  # ACCEPTABLE
                lock_color = Theme.INFO
            
            lock_frame = ctk.CTkFrame(self.results_frame, fg_color=lock_color)
            lock_frame.pack(fill="x", padx=10, pady=10)
            
            # Tiêu đề theo mức độ nghiêm trọng
            if lock_severity == "CRITICAL":
                lock_text = "⛔ KHÓA XUNG NGHIÊM TRỌNG - CẦN XỬ LÝ NGAY" if CURRENT_LANG == "vi" else "⛔ CRITICAL FREQUENCY LOCK - MUST FIX NOW"
            elif lock_severity == "WARNING":
                lock_text = "⚠️ KHÓA XUNG BẤT THƯỜNG - NÊN XỬ LÝ" if CURRENT_LANG == "vi" else "⚠️ ABNORMAL FREQUENCY LOCK - SHOULD FIX"
            elif lock_severity == "MODERATE":
                lock_text = "⚠️ KHÓA XUNG VỪA PHẢI - CÂN NHẮC XỬ LÝ" if CURRENT_LANG == "vi" else "⚠️ MODERATE FREQUENCY LOCK - CONSIDER FIXING"
            else:
                lock_text = "ℹ️ KHÓA XUNG NHẸ - CÓ THỂ CHẤP NHẬN" if CURRENT_LANG == "vi" else "ℹ️ LIGHT FREQUENCY LOCK - ACCEPTABLE"
            ...
            if CURRENT_LANG == "vi":
                ...
                ratio_text = f"• CPU chạy ở {(min_freq/max_freq)*100:.0f}% công suất tối đa"
                cpu_text = "• CPU không tự điều chỉnh tần số theo tải"
            else:
                analysis_text = "📊 Analysis:"
                freq_text = f"• Fixed frequency at {min_freq:.0f}MHz (variation <5%)"
                ratio_text = f"• CPU running at {(min_freq/max_freq)*100:.0f}% of max capacity"
            ...
            ctk.CTkLabel(lock_info_frame, text=ratio_text, font=Theme.SMALL_FONT, text_color="white").pack(anchor="w", padx=20)
            ctk.CTkLabel(lock_info_frame, text=cpu_text, font=Theme.SMALL_FONT, text_color="white").pack(anchor="w", padx=20)
            
            # Đánh giá và khuyến nghị theo mức độ
            if lock_severity == "CRITICAL":
                if CURRENT_LANG == "vi":
                    assess_text = "\n🚨 ĐÁNH GIÁ: NGHIÊM TRỌNG"
                    assess_items = [
                        "• CPU bị giới hạn dưới 1GHz - hiệu năng CỰC KÉM",
                        "• Không thể làm việc bình thường",
                        "• Máy sẽ giật lag nghiêm trọng",
                        "• ⚠️ BẮT BUỘC phải xử lý ngay!"
                    ]
                else:
                    assess_text = "\n🚨 ASSESSMENT: CRITICAL"
                    assess_items = [
                        "• CPU limited below 1GHz - EXTREMELY POOR performance",
                        "• Cannot work normally",
                        "• System will lag severely",
                        "• ⚠️ MUST fix immediately!"
                    ]
            elif lock_severity == "WARNING":
                if CURRENT_LANG == "vi":
                    assess_text = "\n⚠️ ĐÁNH GIÁ: BẤT THƯỜNG"
                    assess_items = [
                        "• CPU chỉ chạy <50% công suất tối đa",
                        "• Hiệu năng bị giảm đáng kể",
                        "• Ảnh hưởng đến trải nghiệm sử dụng",
                        "• 👉 Nên xử lý để tận dụng hiệu năng"
                    ]
                else:
                    assess_text = "\n⚠️ ASSESSMENT: ABNORMAL"
                    assess_items = [
                        "• CPU only running at <50% max capacity",
                        "• Performance significantly reduced",
                        "• Affects user experience",
                        "• 👉 Should fix to utilize full performance"
                    ]
            elif lock_severity == "MODERATE":
                if CURRENT_LANG == "vi":
                    assess_text = "\n⚠️ ĐÁNH GIÁ: VỪA PHẢI"
                    assess_items = [
                        "• CPU chạy ở 50-70% công suất tối đa",
                        "• Hiệu năng bị giảm nhẹ",
                        "• Có thể chấp nhận cho công việc nhẹ",
                        "• 💡 Cân nhắc xử lý nếu cần hiệu năng cao"
                    ]
                else:
                    assess_text = "\n⚠️ ASSESSMENT: MODERATE"
                    assess_items = [
                        "• CPU running at 50-70% max capacity",
                        "• Performance slightly reduced",
                        "• Acceptable for light tasks",
                        "• 💡 Consider fixing if need high performance"
                    ]
            else:  # ACCEPTABLE
                if CURRENT_LANG == "vi":
                    assess_text = "\nℹ️ ĐÁNH GIÁ: CÓ THỂ CHẤP NHẬN"
                    assess_items = [
                        "• CPU chạy ở ≥70% công suất tối đa",
                        "• Hiệu năng vẫn ở mức khá",
                        "• Có thể do chế độ tiết kiệm pin",
                        "• ✅ Không cần xử lý nếu không yêu cầu cao"
                    ]
                else:
                    assess_text = "\nℹ️ ASSESSMENT: ACCEPTABLE"
                    assess_items = [
                        "• CPU running at ≥70% max capacity",
                        "• Performance still quite good",
                        "• May be due to power saving mode",
                        "• ✅ No need to fix if not demanding"
                    ]
            
            ctk.CTkLabel(lock_info_frame, text=assess_text, font=Theme.BODY_FONT, text_color="white").pack(anchor="w")
            for item in assess_items:
                ctk.CTkLabel(lock_info_frame, text=item, font=Theme.SMALL_FONT, text_color="white").pack(anchor="w", padx=20)
            
            # Nguyên nhân và cách khắc phục (chỉ hiển thị nếu cần xử lý)
            if lock_severity in ["CRITICAL", "WARNING", "MODERATE"]:
                if CURRENT_LANG == "vi":
                    causes_text = "\n🔍 Nguyên nhân có thể:"
                ...
                fix_title = "\n💡 Cách khắc phục:" if CURRENT_LANG == "vi" else "\n💡 How to fix:"
                fix1 = "• Kiểm tra BIOS: CPU Frequency → Auto/Dynamic" if CURRENT_LANG == "vi" else "• Check BIOS: CPU Frequency → Auto/Dynamic"
                fix2 = "• Windows: Chế độ nguồn → Hiệu năng cao" if CURRENT_LANG == "vi" else "• Windows: Power mode → High performance"
                fix3 = "• Tắt phần mềm quản lý nguồn của hãng" if CURRENT_LANG == "vi" else "• Disable manufacturer power management software"
                
                ctk.CTkLabel(lock_info_frame, text=fix_title, font=Theme.BODY_FONT, text_color="white").pack(anchor="w")
                ctk.CTkLabel(lock_info_frame, text=fix1, font=Theme.SMALL_FONT, text_color="white").pack(anchor="w", padx=20)
                ctk.CTkLabel(lock_info_frame, text=fix2, font=Theme.SMALL_FONT, text_color="white").pack(anchor="w", padx=20)
                ctk.CTkLabel(lock_info_frame, text=fix3, font=Theme.SMALL_FONT, text_color="white").pack(anchor="w", padx=20)
...
class GPUStressTestStep(BaseStressTestStep):
    def __init__(self, master, **kwargs):
        ...
        if CURRENT_LANG == "vi":
            ...
            how_text = "Nhấn 'Bắt đầu Test' để chạy test 60 giây. Cửa sổ test sẽ hiển thị đồ họa nặng nhưng KHÔNG toàn màn hình để bạn có thể nhấn 'Dừng Test' hoặc ESC bất cứ lúc nào. Quan sát có hiện tượng chớp giật, sọc ngang, hay đốm màu lạ không."
        else:
            ...
            how_text = "Click 'Start Test' to run 60-second test. Test window will show heavy graphics but NOT fullscreen so you can click 'Stop Test' or ESC anytime. Watch for flickering, lines, or color artifacts."
        super().__init__(master, title, why_text, how_text, **kwargs)
        self.TEST_DURATION = 60
        self.estimated_time = "1-2 phút" if CURRENT_LANG == "vi" else "1-2 mins"
        self.test_running = False
    ...
    def stop_test(self):
        """Dừng GPU stress test"""
        self.test_running = False
        # Signal to worker to stop via queue
        try:
            self.queue.put({'type': 'stop'})
        except:
            pass
        # Call parent's stop_test to handle process termination
        super().stop_test()
        # Additional GPU-specific cleanup
        if pygame:
            try:
                pygame.quit()
            except:
...
class HardDriveSpeedStep(BaseStressTestStep):
    ...
    def finalize_test(self, msg):
        ...
        header_text = "Kết quả Test Ổ Cứng:" if CURRENT_LANG == "vi" else "Disk Test Results:"
        write_label = "Tốc độ Ghi:" if CURRENT_LANG == "vi" else "Write Speed:"
        read_label = "Tốc độ Đọc:" if CURRENT_LANG == "vi" else "Read Speed:"
        
        ctk.CTkLabel(self.results_frame, text=header_text, font=Theme.SUBHEADING_FONT).pack(pady=10)
        ctk.CTkLabel(self.results_frame, text=f"{write_label} {write_speed} MB/s", font=Theme.BODY_FONT).pack()
        ctk.CTkLabel(self.results_frame, text=f"{read_label} {read_speed} MB/s", font=Theme.BODY_FONT).pack()
        ...
        speed_good_text = "Tốc độ tốt" if CURRENT_LANG == "vi" else "Speed Good"
        ctk.CTkButton(button_bar, text=speed_good_text, command=lambda: self.mark_completed({"Kết quả": write_read_text, "Trạng thái": get_text("status_good")}, auto_advance=True), fg_color=Theme.SUCCESS).pack(side="left", padx=10)
...
class PhysicalInspectionStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        kwargs["step_key"] = "physical_inspection"
...
class BIOSCheckStep(BaseStepFrame):
    ...
    def create_bios_checklist(self):
        ...
        for item in access_items:
            ctk.CTkLabel(access_frame, text=item, font=Theme.SMALL_FONT, text_color=Theme.TEXT_SECONDARY).pack(anchor="w", padx=25, pady=1)
        
        # Hiệu năng CPU
        cpu_frame = ctk.CTkFrame(checklist_frame, fg_color=Theme.BACKGROUND)
        cpu_frame.pack(fill="x", padx=15, pady=5)
        cpu_label = "⚡ Hiệu Năng CPU:" if CURRENT_LANG == "vi" else "⚡ CPU Performance:"
        ctk.CTkLabel(cpu_frame, text=cpu_label, font=Theme.BODY_FONT, text_color=Theme.ACCENT).pack(anchor="w", padx=10, pady=5)
        if CURRENT_LANG == "vi":
            cpu_items = ["✓ Intel Turbo Boost: Phải 'Enabled'", "✓ AMD Turbo Core/Precision Boost: Phải 'Enabled'", "✓ CPU C-States: Nên 'Enabled' (tiết kiệm pin)", "✓ Virtualization (VT-x/AMD-V): 'Enabled' nếu dùng VM"]
        else:
            cpu_items = ["✓ Intel Turbo Boost: Must be 'Enabled'", "✓ AMD Turbo Core/Precision Boost: Must be 'Enabled'", "✓ CPU C-States: Should be 'Enabled' (power saving)", "✓ Virtualization (VT-x/AMD-V): 'Enabled' if using VM"]
        ...
        if CURRENT_LANG == "vi":
            ram_items = ["✓ XMP/DOCP Profile: Nên bật (RAM chạy đúng tốc độ)", "✓ Kiểm tra dung lượng RAM hiển thị khớp thực tế"]
        else:
            ram_items = ["✓ XMP/DOCP Profile: Should enable (RAM runs at correct speed)", "✓ Check RAM capacity matches actual amount"]
        ...
        if CURRENT_LANG == "vi":
            ...
            warning_items = [
                "🚫 CHECK IMMEDIATELY IN BIOS:",
                "",
                "1️⃣ Supervisor/Admin Password: Must be EMPTY",
                "   → If set: DON'T BUY or demand HUGE discount",
                "",
                "2️⃣ HDD/SSD Password: Must be EMPTY",
                "   → If set: Drive is LOCKED, unusable!",
                "",
                "🚨 THINKPAD SPECIAL:",
                "   • ThinkPad BIOS password is NEARLY IMPOSSIBLE to crack",
                "   • Must replace Motherboard ($500-1000)",
                "   • Or send to Lenovo ($300-500 + long wait)",
                "   • If HDD locked: Drive becomes brick!",
                "",
                "❌ If password set: REFUSE to buy!"
            ]
        ...
        security_label = "🔒 BẢO MẬT KHÁC:" if CURRENT_LANG == "vi" else "🔒 OTHER SECURITY:"
        ctk.CTkLabel(security_frame, text=security_label, font=Theme.BODY_FONT, text_color="#FCA5A5").pack(anchor="w", padx=10, pady=5)
        if CURRENT_LANG == "vi":
            security_items = ["⚠️ Computrace/Absolute: Phải 'Disabled' hoặc 'Inactive'", "⚠️ Intel AMT/vPro: Nên 'Disabled' (trừ doanh nghiệp)", "✓ Secure Boot: 'Enabled' cho bảo mật", "✓ TPM: 'Enabled' (Windows 11 yêu cầu)"]
        else:
            security_items = ["⚠️ Computrace/Absolute: Must be 'Disabled' or 'Inactive'", "⚠️ Intel AMT/vPro: Should be 'Disabled' (except enterprise)", "✓ Secure Boot: 'Enabled' for security", "✓ TPM: 'Enabled' (Windows 11 requires)"]
        ...
        thinkpad_frame = ctk.CTkFrame(checklist_frame, fg_color="#1E3A8A", border_width=2, border_color="#3B82F6")
        thinkpad_frame.pack(fill="x", padx=15, pady=10)
        thinkpad_label = "🔴 THINKPAD - CÀI ĐẶT KHÁC:" if CURRENT_LANG == "vi" else "🔴 THINKPAD - OTHER SETTINGS:"
        ctk.CTkLabel(thinkpad_frame, text=thinkpad_label, font=Theme.BODY_FONT, text_color="#60A5FA").pack(anchor="w", padx=10, pady=5)
        if CURRENT_LANG == "vi":
            thinkpad_items = ["✓ Battery Charge Threshold: 80% (kéo dài tuổi thọ pin)", "✓ Rapid Charge: 'Enabled' (sạc nhanh)", "✓ Always On USB: Tùy nhu cầu", "✓ Fn/Ctrl Key Swap: Tùy thích"]
        else:
            thinkpad_items = ["✓ Battery Charge Threshold: 80% (extends battery life)", "✓ Rapid Charge: 'Enabled' (fast charging)", "✓ Always On USB: As needed", "✓ Fn/Ctrl Key Swap: Personal preference"]
        for item in thinkpad_items:
            ctk.CTkLabel(thinkpad_frame, text=item, font=Theme.SMALL_FONT, text_color="white").pack(anchor="w", padx=25, pady=1)
...
class CPUStressTestStep(BaseStepFrame):
    ...
    def create_cpu_test(self):
        ...
        cpu_title = "CPU Stress Test" if CURRENT_LANG == "en" else "Test Tải CPU"
        ctk.CTkLabel(control_frame, text=cpu_title, font=Theme.SUBHEADING_FONT, text_color=Theme.ACCENT).pack(pady=10)
...
class GPUStressTestStep(BaseStepFrame):
    ...
    def create_gpu_test(self):
        control_frame = ctk.CTkFrame(self.action_frame, fg_color=Theme.FRAME, corner_radius=Theme.CORNER_RADIUS)
        control_frame.grid(row=0, column=0, sticky="ew", padx=20, pady=10)
        gpu_title = "GPU Stress Test" if CURRENT_LANG == "en" else "Test Tải GPU"
        ctk.CTkLabel(control_frame, text=gpu_title, font=Theme.SUBHEADING_FONT, text_color=Theme.ACCENT).pack(pady=10)
...
class BatteryHealthStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        kwargs["step_key"] = "battery_health"
        ...
        try:
            ...
            if battery:
                ...
                if health_percent > 80:
                    ...
                    condition = "Trung bình" if CURRENT_LANG == "vi" else "Fair"
                    condition_color = Theme.WARNING
                    condition_icon = "⚠️"
                else:
                    condition = "Yếu" if CURRENT_LANG == "vi" else "Poor"
                ...
                if health_percent < 60:
                    ...
                    status_label = "⚠️ TÌNH TRẠNG: Pin suy giảm nghiêm trọng" if CURRENT_LANG == "vi" else "⚠️ CONDITION: Battery severely degraded"
                    ctk.CTkLabel(status_frame, text=status_label, font=Theme.BODY_FONT, text_color="white").pack(pady=5)
                    if CURRENT_LANG == "vi":
                        status_recs = ["• Nên thay pin mới để đảm bảo an toàn", "• Thời gian sử dụng rất ngắn, có thể tắt máy đột ngột", "• Pin cũ có nguy cơ phồng rộp, cần kiểm tra ngay"]
                    else:
                        status_recs = ["• Should replace battery for safety", "• Very short runtime, may shutdown unexpectedly", "• Old battery may swell, inspect immediately"]
                    ...
                    status_frame.pack(fill="x", padx=15, pady=5)
                    status_label = "⚠️ TÌNH TRẠNG: Pin bắt đầu suy giảm" if CURRENT_LANG == "vi" else "⚠️ CONDITION: Battery starting to degrade"
                    ctk.CTkLabel(status_frame, text=status_label, font=Theme.BODY_FONT, text_color="white").pack(pady=5)
                    if CURRENT_LANG == "vi":
                        status_recs = ["• Pin còn dùng được nhưng cần chăm sóc kỹ hơn", "• Thời gian sử dụng giảm so với lúc mới", "• Nên chuẩn bị thay pin trong 6-12 tháng tới"]
                    else:
                        status_recs = ["• Battery still usable but needs better care", "• Runtime reduced compared to new", "• Prepare to replace in 6-12 months"]
                    for rec in status_recs:
                        ctk.CTkLabel(status_frame, text=rec, font=Theme.SMALL_FONT, text_color="white").pack(anchor="w", padx=10, pady=1)
                else:
                    status_frame = ctk.CTkFrame(rec_frame, fg_color=Theme.SUCCESS)
                    status_frame.pack(fill="x", padx=15, pady=5)
                    status_label = "✅ TÌNH TRẠNG: Pin còn tốt" if CURRENT_LANG == "vi" else "✅ CONDITION: Battery in good shape"
                    ctk.CTkLabel(status_frame, text=status_label, font=Theme.BODY_FONT, text_color="white").pack(pady=5)
                    if CURRENT_LANG == "vi":
                        status_recs = ["• Pin đang trong tình trạng tốt", "• Tiếp tục chăm sóc đúng cách để kéo dài tuổi thọ"]
                    else:
                        status_recs = ["• Battery in good condition", "• Continue proper care to extend lifespan"]
                    for rec in status_recs:
                        ctk.CTkLabel(status_frame, text=rec, font=Theme.SMALL_FONT, text_color="white").pack(anchor="w", padx=10, pady=1)
                ...
                if CURRENT_LANG == "vi":
                    tips = [
                        "✓ Giữ pin ở mức 20-80% để kéo dài tuổi thọ (tránh sạc 100% liên tục)",
                        "✓ Rút sạc khi đã đầy nếu không sử dụng lâu dài",
                        "✓ Tránh để pin xuống dưới 20% thường xuyên",
                        "✓ Sử dụng sạc chính hãng hoặc có chứng nhận an toàn",
                        "✓ Tránh để máy nóng khi sạc (không đặt trên chăn, gối)",
                        "✓ Nếu không dùng laptop lâu (>1 tháng), sạc pin 50-60% rồi tắt máy"
                    ]
                else:
                    tips = [
                        "✓ Keep battery between 20-80% for longevity (avoid 100% constantly)",
                        "✓ Unplug when fully charged if not using for long",
                        "✓ Avoid letting battery drop below 20% frequently",
                        "✓ Use original or certified chargers only",
                        "✓ Avoid heat while charging (don't place on blankets/pillows)",
                        "✓ If not using laptop >1 month, charge to 50-60% then shutdown"
                    ]
                ...
                if CURRENT_LANG == "vi":
                    ...
                    avoid = [
                        "✗ Charging overnight frequently (battery at 100% too long)",
                        "✗ Letting battery drain to 0% before charging",
                        "✗ Using laptop while charging with heavy tasks (gaming, rendering)",
                        "✗ Keeping laptop in hot (>35°C) or cold (<0°C) places",
                        "✗ Using low-quality chargers from unknown sources",
                        "✗ Plugging in 24/7 for months"
                    ]
    ...
    def show_result_choices(self):
        ...
        if hasattr(self, 'battery_health'):
            if self.battery_health > 80:
                suggested_text = "✓ Pin Tốt (>80%)" if CURRENT_LANG == "vi" else "✓ Battery Good (>80%)"
                suggested_color = Theme.SUCCESS
            elif self.battery_health > 60:
                suggested_text = "⚠️ Pin Trung Bình (60-80%)" if CURRENT_LANG == "vi" else "⚠️ Battery Fair (60-80%)"
                suggested_color = Theme.WARNING
            else:
                suggested_text = "❌ Pin Yếu (<60%)" if CURRENT_LANG == "vi" else "❌ Battery Poor (<60%)"
                suggested_color = Theme.ERROR
        else:
            suggested_text = "✓ Pin Tốt" if CURRENT_LANG == "vi" else "✓ Battery Good"
            suggested_color = Theme.SUCCESS
...
class AudioTestStep(BaseStepFrame):
    ...
    def create_audio_tests(self):
        ...
        # Speaker tests
        speaker_title = "Speaker Test:" if CURRENT_LANG == "en" else "Test Loa:"
        ctk.CTkLabel(test_frame, text=speaker_title, font=Theme.SUBHEADING_FONT).pack(pady=10)
...
class WebcamTestStep(BaseStepFrame):
    ...
    def create_webcam_test(self):
        ...
        webcam_title = "Webcam Test" if CURRENT_LANG == "en" else "Kiểm Tra Webcam"
        ctk.CTkLabel(test_frame, text=webcam_title, font=Theme.SUBHEADING_FONT, text_color=Theme.ACCENT).pack(pady=10)
    ...
    def start_camera_test(self):
        if not cv2:
            opencv_error = "❌ OpenCV không có sẵn. Cài đặt: pip install opencv-python" if CURRENT_LANG == "vi" else "❌ OpenCV not available. Install: pip install opencv-python"
            self.status_label.configure(text=opencv_error, text_color=Theme.ERROR)
            return
        ...
        try:
            ...
            if not self.cap:
                no_camera = "❌ Không tìm thấy camera hoạt động" if CURRENT_LANG == "vi" else "❌ No working camera found"
                self.status_label.configure(text=no_camera, text_color=Theme.ERROR)
                return
            ...
            resolution_text = f"📷 Độ phân giải tối đa: {actual_width}x{actual_height}" if CURRENT_LANG == "vi" else f"📷 Max resolution: {actual_width}x{actual_height}"
            self.preview_label.configure(text=resolution_text)
            
            self.start_video_preview()
            
        except Exception as e:
            error_text = f"❌ Lỗi camera: {e}" if CURRENT_LANG == "vi" else f"❌ Camera error: {e}"
            self.status_label.configure(text=error_text, text_color=Theme.ERROR)
        ...
        if hasattr(self, 'cap') and self.cap and self.camera_running:
            ...
            if ret and frame is not None:
                ...
                if self.obstruction_detected:
                    ...
                    obstruction_text = "PHÁT HIỆN VẬT CẢN!" if CURRENT_LANG == "vi" else "OBSTRUCTION DETECTED!"
                    cv2.putText(frame, obstruction_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                else:
                ...
                if self.frame_count % 30 == 0:  # Update every 30 frames
                    if CURRENT_LANG == "vi":
                        ...
                        obstruction_status = " - ⚠️ BLOCKED" if self.obstruction_detected else " - ✅ CLEAR"
...
class NetworkTestStep(BaseStepFrame):
    ...
    def create_network_test(self):
        ...
        btn_frame = ctk.CTkFrame(test_frame, fg_color="transparent")
        btn_frame.pack(pady=10)
        self.start_btn = ctk.CTkButton(btn_frame, text=get_text("start_test_btn"), command=self.start_network_test, fg_color=Theme.ACCENT, height=Theme.BUTTON_HEIGHT)
        self.start_btn.pack(side="left", padx=5)
        self.stop_btn = ctk.CTkButton(btn_frame, text=get_text("stop_test_btn"), command=self.stop_network_test, fg_color=Theme.ERROR, height=Theme.BUTTON_HEIGHT, state="disabled")
        self.stop_btn.pack(side="left", padx=5)
        self.status_label = ctk.CTkLabel(test_frame, text=get_text("ready_to_test"), font=Theme.BODY_FONT)
        self.status_label.pack(pady=5)
    ...
    def run_network_tests(self):
        ...
        for i, (name, func) in enumerate(tests):
            ...
            time.sleep(0.5)
        if self.is_testing:
            completed_text = "Network test completed!" if CURRENT_LANG == "en" else "Test mạng hoàn thành!"
            self.status_label.configure(text=completed_text)
            self.is_testing = False
            self.start_btn.configure(state="normal")
            self.stop_btn.configure(state="disabled")
            self.start_btn.configure(state="normal")
...
class ThermalMonitorStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        kwargs["step_key"] = "thermal_monitor"
    ...
    def update_ui(self, cpu_temp, cpu_usage, memory_usage):
        ...
        if hasattr(self, 'cpu_temp_label') and self.cpu_temp_label.winfo_exists():
            self.cpu_temp_label.configure(text=f"🌡️ CPU: {cpu_temp:.1f}°C", text_color=temp_color)
        if hasattr(self, 'cpu_usage_label') and self.cpu_usage_label.winfo_exists():
    ...
    def show_result_choices(self):
        for widget in self.result_container.winfo_children():
            widget.destroy()
        thermal_question = "Thermal performance OK?" if CURRENT_LANG == "en" else "Hiệu năng nhiệt độ ổn định không?"
        ctk.CTkLabel(self.result_container, text=thermal_question, font=Theme.SUBHEADING_FONT).pack(pady=15)
        button_bar = ctk.CTkFrame(self.result_container, fg_color="transparent")
        button_bar.pack(pady=15)
...
class SystemStabilityStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        kwargs["step_key"] = "system_stability"
    ...
    def create_stability_test(self):
        ...
        combined_title = "🔥 Combined Stress Test" if CURRENT_LANG == "en" else "🔥 Test Tải Kết Hợp"
        ctk.CTkLabel(test_frame, text=combined_title, font=Theme.SUBHEADING_FONT, text_color=Theme.ACCENT).pack(pady=10)
        
        # Button container for Start and Stop buttons
        button_container = ctk.CTkFrame(test_frame, fg_color="transparent")
        button_container.pack(pady=10)
        
        time_text = " (3-5 mins)" if CURRENT_LANG == "en" else " (3-5 phút)"
        self.start_btn = ctk.CTkButton(button_container, text=get_text("start_test_btn") + time_text, command=self.start_test, fg_color=Theme.ACCENT, height=Theme.BUTTON_HEIGHT)
        self.start_btn.pack(side="left", padx=5)
        
        self.stop_btn = ctk.CTkButton(button_container, text=get_text("stop_test_btn"), command=self.stop_test, fg_color=Theme.WARNING, height=Theme.BUTTON_HEIGHT, state="disabled")
        self.stop_btn.pack(side="left", padx=5)
    ...
    def stop_test(self):
        """Dừng combined stress test"""
        self.is_testing = False
        self.stop_btn.configure(state="disabled")
        self.start_btn.configure(state="normal")
        stop_msg = t("Test đã dừng bởi người dùng") if CURRENT_LANG == "vi" else "Test stopped by user"
        self.status_label.configure(text=stop_msg, text_color=Theme.WARNING)
    ...
    def run_combined_test(self):
        ...
        while time.time() - start_time < duration and self.is_testing:
            ...
            # Format time display with translation
            time_elapsed = int(elapsed)
            time_total = int(duration)
            if CURRENT_LANG == "vi":
...
class SummaryStep(BaseStepFrame):
    ...
    def create_quick_guide(self, parent):
        guide_frame = ctk.CTkFrame(parent, fg_color=Theme.INFO, corner_radius=8)
        guide_frame.pack(fill="x", pady=(0, 15))
        
        title = "📖 Quick Guide" if CURRENT_LANG == "en" else "📖 Hướng Dẫn Nhanh"
        ctk.CTkLabel(guide_frame, text=title, font=Theme.SUBHEADING_FONT, text_color="white").pack(pady=(10,5))
        
        if CURRENT_LANG == "vi":
            tips = [
                "✅ XEM KẾT QUẢ: Cuộn xuống để xem báo cáo chi tiết",
                "📊 PHÂN TÍCH: Kiểm tra từng mục - Bảo mật, Hiệu năng, Giao diện, Phần cứng",
                "⚠️ CHÚ Ý: Mục đỏ/vàng cần kiểm tra kỹ trước khi mua",
                "💾 XUẤT BÁO CÁO: Dùng nút Export ở cuối trang",
                "🔧 CÔNG CỤ BỔ SUNG: Xem danh sách công cụ chuyên nghiệp ở cuối"
            ]
        else:
            tips = [
                "✅ VIEW RESULTS: Scroll down for detailed report",
                "📊 ANALYSIS: Check each category - Security, Performance, Interface, Hardware",
                "⚠️ ATTENTION: Red/yellow items need careful inspection before purchase",
                "💾 EXPORT REPORT: Use Export button at bottom",
                "🔧 ADDITIONAL TOOLS: See professional tools list at bottom"
            ]
        
        for tip in tips:
            ctk.CTkLabel(guide_frame, text=tip, font=Theme.SMALL_FONT, text_color="white", anchor="w").pack(anchor="w", padx=20, pady=2)
        
        ctk.CTkLabel(guide_frame, text="", font=Theme.SMALL_FONT).pack(pady=5)
    ...
    def create_simple_summary(self, results):
        ...
        if not results:
            ...
            # Categorize results using step_key (constant identifiers, not translated titles)
            categories = {
            ...
            for category, test_keys in categories.items():
                ...
                for test_key in test_keys:
                    if test_key in results:
                        ...
                        # Get display name from result data
                        test_name = result.get("Tên test", get_text(test_key))
                        ...
                        # Create result card for each test
                        result_card = ctk.CTkFrame(category_frame, fg_color=Theme.CARD, corner_radius=6, border_width=2, border_color=status_color)
                        result_card.pack(fill="x", padx=20, pady=5)
                        
                        result_content = ctk.CTkFrame(result_card, fg_color="transparent")
                        result_content.pack(fill="x", padx=12, pady=10)
                        
                        # Test name and status
                        header_frame = ctk.CTkFrame(result_content, fg_color="transparent")
                        header_frame.pack(fill="x")
                        
                        ctk.CTkLabel(header_frame, text=f"{status_icon} {test_name}", font=("Arial", 20, "bold"), text_color=Theme.TEXT).pack(side="left")
                        ctk.CTkLabel(header_frame, text=status, font=("Arial", 18, "bold"), text_color=status_color).pack(side="right")
                        
                        # Result details
                        if result.get("Kết quả"):
                            ctk.CTkLabel(result_content, text=f"Kết quả: {result['Kết quả']}", font=("Arial", 17), text_color=Theme.TEXT_SECONDARY, wraplength=900, justify="left").pack(anchor="w", pady=(5,0))
                        
                        # Additional details if available
                        if result.get("Chi tiết"):
                            details_text = result['Chi tiết'][:200] + "..." if len(result.get('Chi tiết', '')) > 200 else result.get('Chi tiết', '')
                            ctk.CTkLabel(result_content, text=f"Chi tiết: {details_text}", font=("Arial", 15), text_color=Theme.TEXT_SECONDARY, wraplength=900, justify="left").pack(anchor="w", pady=(3,0))
        ...
        tools_desc_text = "Để xác minh kết quả AI và kiểm tra sâu hơn, hãy sử dụng các công cụ chuyên nghiệp sau:" if CURRENT_LANG == "vi" else "To verify AI results and inspect deeper, use these professional tools:"
        ctk.CTkLabel(tools_frame, text=tools_desc_text, font=Theme.BODY_FONT, text_color=Theme.TEXT_SECONDARY).pack(padx=20)
        
        verification_text = "💡 Khuyến khích so sánh kết quả với các tools này để đảm bảo độ chính xác cao nhất." if CURRENT_LANG == "vi" else "💡 Recommended to compare results with these tools for highest accuracy."
        ...
        if CURRENT_LANG == "vi":
            ...
            tools = [
                ("💾 CrystalDiskInfo", "Detailed S.M.A.R.T hard drive check", "winget install CrystalDewWorld.CrystalDiskInfo", "https://crystalmark.info/en/software/crystaldiskinfo/"),
                ("🌡️ HWiNFO64", "Monitor temperature and sensors", "winget install REALiX.HWiNFO", "https://www.hwinfo.com/"),
                ("⚡ CPU-Z", "CPU, RAM, Motherboard info", "winget install CPUID.CPU-Z", "https://www.cpuid.com/softwares/cpu-z.html"),
                ("🎮 GPU-Z", "Detailed graphics card info", "winget install TechPowerUp.GPU-Z", "https://www.techpowerup.com/gpuz/"),
                ("🔥 FurMark", "Professional GPU stress test", "winget install Geeks3D.FurMark", "https://geeks3d.com/furmark/"),
                ("🧠 MemTest86", "RAM error check (USB boot)", "Download from homepage", "https://www.memtest86.com/")
            ]
...
class WizardFrame(ctk.CTkFrame):
    ...
    def _get_steps_for_mode(self, mode):
        basic_steps = [
            ...
            (get_text("bios_check"), BIOSCheckStep),
        ...
        expert_steps = basic_steps + [
            ...
            (get_text("thermal_monitor"), ThermalMonitorStep),
            (get_text("system_stability"), SystemStabilityStep)
        ]
        
        return expert_steps if mode == "expert" else basic_steps
    
    def show_step(self, step_index):
        # Cleanup keyboard hooks trước khi chuyển step
        if keyboard:
            try:
                keyboard.unhook_all()
                keyboard.unblock_key('print screen')
    ...
    def record_result(self, step_name, result_data):
        # Use step_key for consistent storage across language changes
        key = getattr(self, 'step_key', step_name) if hasattr(self, 'step_key') else step_name
        self.all_results[key] = result_data
...
class IndividualTestFrame(ctk.CTkFrame):
    def __init__(self, master, icon_manager, app=None):
        ...
        # Container for scrollable test list with scroll buttons
...
class IntroductionFrame(ctk.CTkFrame):
    def __init__(self, master, icon_manager, app=None):
        ...
        scroll_frame = ctk.CTkScrollableFrame(scroll_container, fg_color=Theme.BACKGROUND)
        scroll_frame.grid(row=0, column=0, sticky="nsew")
        scroll_frame.grid_columnconfigure(0, weight=1)
        
        # Header with back button
        header = ctk.CTkFrame(scroll_frame, fg_color=Theme.FRAME, corner_radius=Theme.CORNER_RADIUS)
        header.pack(fill="x", pady=(0, 20))
        
        back_btn_text = "🏠 Trang chủ" if CURRENT_LANG == "vi" else "🏠 Home"
        ctk.CTkButton(header, text=back_btn_text, command=lambda: app.show_mode_selection() if app else None,
                     fg_color=Theme.ACCENT, hover_color=Theme.ACCENT_HOVER, height=32, font=Theme.BUTTON_FONT).pack(side="left", padx=10, pady=10)
        
        title_text = "📖 Giới Thiệu LaptopTester Pro" if CURRENT_LANG == "vi" else "📖 Introduction to LaptopTester Pro"
        ctk.CTkLabel(header, text=title_text, font=Theme.HEADING_FONT, text_color=Theme.ACCENT).pack(side="left", padx=20, pady=10)
        
        # Content
        content = ctk.CTkFrame(scroll_frame, fg_color=Theme.FRAME, corner_radius=Theme.CORNER_RADIUS)
        content.pack(fill="both", expand=True, pady=10)
        
        # Introduction text - Language-specific
        if CURRENT_LANG == "vi":
            intro_sections = [
                {
                    "title": "🎯 Về LaptopTester Pro",
                    "content": "LaptopTester Pro là công cụ kiểm tra laptop toàn diện, chuyên nghiệp được phát triển để giúp bạn:\n\n"
                               "✅ Kiểm tra phần cứng laptop trước khi mua (laptop cũ, laptop đã qua sử dụng)\n"
                               "✅ Đánh giá tình trạng laptop hiện tại\n"
                               "✅ Phát hiện các lỗi tiềm ẩn về phần cứng\n"
                               "✅ Stress test để kiểm tra độ ổn định hệ thống\n"
                               "✅ Tạo báo cáo chi tiết về tình trạng laptop"
                },
                {
                    "title": "🌟 Tính Năng Nổi Bật",
                    "content": "🔒 Bảo mật & Giấy phép:\n"
                               "   • Đọc thông tin phần cứng trực tiếp từ BIOS (khó làm giả)\n"
                               "   • Kiểm tra bản quyền Windows\n"
                               "   • So sánh thông tin Windows vs BIOS\n\n"
                               "⚙️ Kiểm tra Phần cứng:\n"
                               "   • Sức khỏe ổ cứng (S.M.A.R.T)\n"
                               "   • Tốc độ đọc/ghi ổ cứng\n"
                               "   • Sức khỏe pin & chu kỳ sạc\n"
                               "   • Màn hình (dead pixel, color test)\n"
                               "   • Bàn phím & Touchpad\n"
                               "   • Webcam, Loa, Micro\n"
                               "   • Kết nối mạng WiFi/Ethernet\n\n"
                               "🔥 Stress Test:\n"
                               "   • CPU Stress Test với giám sát nhiệt độ\n"
                               "   • GPU Stress Test với hiệu ứng đồ họa\n"
                               "   • RAM Test\n"
                               "   • Test tổng hợp CPU+GPU+RAM\n\n"
                               "📊 Báo cáo & Phân tích:\n"
                               "   • Báo cáo chi tiết từng bước test\n"
                               "   • Đánh giá AI về tình trạng laptop\n"
                               "   • Xuất báo cáo PDF/Excel\n"
                               "   • Copy báo cáo dạng text"
                },
                {
                    "title": "🚀 Cách Sử Dụng",
                    "content": "1️⃣ Chế độ Cơ Bản:\n"
                               "   • Dành cho người dùng thông thường\n"
                               "   • Kiểm tra nhanh các chức năng chính\n"
                               "   • Thời gian: 15-20 phút\n\n"
                               "2️⃣ Chế độ Chuyên Gia:\n"
                               "   • Dành cho người am hiểu kỹ thuật\n"
                               "   • Bao gồm tất cả test + stress test\n"
                               "   • Thời gian: 30-40 phút\n\n"
                               "3️⃣ Kiểm Tra Riêng Lẻ:\n"
                               "   • Chọn từng thành phần để kiểm tra\n"
                               "   • Linh hoạt, không cần làm theo thứ tự\n"
                               "   • Thời gian: Tùy chỉnh"
                },
                {
                    "title": "⚠️ Lưu Ý Quan Trọng",
                    "content": "🔴 Khi mua laptop cũ:\n"
                               "   • Luôn chạy test trực tiếp trên máy (không tin ảnh/video)\n"
                               "   • Chú ý đến các test: BIOS, S.M.A.R.T, Battery Health\n"
                               "   • So sánh thông tin quảng cáo vs thực tế\n\n"
                               "🟡 Stress Test:\n"
                               "   • Laptop sẽ chạy ở công suất cao\n"
                               "   • Nhiệt độ có thể tăng cao (bình thường)\n"
                               "   • Không nên chạy quá lâu nếu laptop cũ\n\n"
                               "🟢 Kết quả test:\n"
                               "   • Tham khảo, không thay thế kiểm tra chuyên nghiệp\n"
                               "   • Nên so sánh với công cụ khác (CrystalDiskInfo, HWiNFO)\n"
                               "   • Lưu báo cáo để so sánh sau này"
                },
                {
                    "title": "👨‍💻 Về Tác Giả",
                    "content": "💻 Phát triển bởi: Laptop Lê Ẩn & Gemini AI\n"
                               "📍 Địa chỉ: 371 Nguyễn Kiệm, Phường 3, Gò Vấp, TP.HCM\n"
                               "📱 Hotline: 0931.78.79.32\n"
                               "🌐 Facebook: fb.com/maytinh371nguyenkiem\n"
                               "🛒 Shopee: s.shopee.vn/7AUkbxe8uu\n\n"
                               "❤️ Cảm ơn bạn đã sử dụng LaptopTester Pro!\n"
                               "💬 Mọi góp ý xin liên hệ qua Hotline hoặc Facebook."
                }
            ]
        else:  # English
            intro_sections = [
                {
                    ...
                    "content": "LaptopTester Pro is a comprehensive, professional laptop testing tool developed to help you:\n\n"
                               "✅ Test laptop hardware before purchasing (used/refurbished laptops)\n"
                               "✅ Evaluate current laptop condition\n"
                               "✅ Detect potential hardware issues\n"
                               "✅ Stress test to check system stability\n"
                               "✅ Generate detailed laptop condition reports"
                },
                {
                    "title": "🌟 Key Features",
                    "content": "🔒 Security & Licensing:\n"
                               "   • Read hardware info directly from BIOS (hard to fake)\n"
                               "   • Check Windows license status\n"
                               "   • Compare Windows vs BIOS information\n\n"
                               "⚙️ Hardware Testing:\n"
                               "   • Hard drive health (S.M.A.R.T)\n"
                               "   • Read/write speed test\n"
                               "   • Battery health & charge cycles\n"
                               "   • Screen (dead pixel, color test)\n"
                               "   • Keyboard & Touchpad\n"
                               "   • Webcam, Speaker, Microphone\n"
                               "   • WiFi/Ethernet connectivity\n\n"
                               "🔥 Stress Testing:\n"
                               "   • CPU Stress Test with temperature monitoring\n"
                               "   • GPU Stress Test with graphics effects\n"
                               "   • RAM Test\n"
                               "   • Combined CPU+GPU+RAM test\n\n"
                               "📊 Reports & Analysis:\n"
                               "   • Detailed step-by-step test reports\n"
                               "   • AI evaluation of laptop condition\n"
                               "   • Export reports as PDF/Excel\n"
                               "   • Copy report as text"
                },
                {
                    "title": "🚀 How to Use",
                    "content": "1️⃣ Basic Mode:\n"
                               "   • For general users\n"
                               "   • Quick test of main functions\n"
                               "   • Duration: 15-20 minutes\n\n"
                               "2️⃣ Expert Mode:\n"
                               "   • For technical users\n"
                               "   • Includes all tests + stress tests\n"
                               "   • Duration: 30-40 minutes\n\n"
                               "3️⃣ Individual Testing:\n"
                               "   • Choose specific components to test\n"
                               "   • Flexible, no need to follow order\n"
                               "   • Duration: Customizable"
                },
                {
                    "title": "⚠️ Important Notes",
                    "content": "🔴 When buying used laptops:\n"
                               "   • Always run tests directly on the machine (don't trust photos/videos)\n"
                               "   • Pay attention to: BIOS, S.M.A.R.T, Battery Health tests\n"
                               "   • Compare advertised specs vs actual results\n\n"
                               "🟡 Stress Testing:\n"
                               "   • Laptop will run at high performance\n"
                               "   • Temperature may increase significantly (normal)\n"
                               "   • Don't run too long on old laptops\n\n"
                               "🟢 Test Results:\n"
                               "   • For reference, not a replacement for professional inspection\n"
                               "   • Compare with other tools (CrystalDiskInfo, HWiNFO)\n"
                               "   • Save reports for future comparison"
                },
                {
                    "title": "👨‍💻 About the Author",
                    "content": "💻 Developed by: Laptop Le An & Gemini AI\n"
                               "📍 Address: 371 Nguyen Kiem, Ward 3, Go Vap, HCMC\n"
                               "📱 Hotline: 0931.78.79.32\n"
                               "🌐 Facebook: fb.com/maytinh371nguyenkiem\n"
                               "🛒 Shopee: s.shopee.vn/7AUkbxe8uu\n\n"
                               "❤️ Thank you for using LaptopTester Pro!\n"
                               "💬 For feedback, please contact via Hotline or Facebook."
                }
            ]
        
        
        for section in intro_sections:
            section_frame = ctk.CTkFrame(content, fg_color=Theme.CARD, corner_radius=8)
            section_frame.pack(fill="x", padx=20, pady=10)
            
            ctk.CTkLabel(section_frame, text=section["title"], font=("Arial", 18, "bold"), 
                        text_color=Theme.ACCENT, anchor="w").pack(fill="x", padx=20, pady=(15, 5))
            
            ctk.CTkLabel(section_frame, text=section["content"], font=Theme.BODY_FONT,
                        text_color=Theme.TEXT, anchor="w", justify="left", wraplength=1200).pack(fill="x", padx=20, pady=(5, 15))
...
class GuideFrame(ctk.CTkFrame):
    def __init__(self, master, icon_manager, app=None):
        ...
        # Container for scrollable frame with scroll buttons
        scroll_container = ctk.CTkFrame(self, fg_color="transparent")
        scroll_container.grid(row=0, column=0, sticky="nsew", padx=20, pady=20)
        scroll_container.grid_columnconfigure(0, weight=1)
        ...
        ctk.CTkLabel(header, text=title_text, font=Theme.HEADING_FONT, text_color=Theme.WARNING).pack(side="left", padx=20, pady=10)
        
        # Content
        content = ctk.CTkFrame(scroll_frame, fg_color=Theme.FRAME, corner_radius=Theme.CORNER_RADIUS)
        content.pack(fill="both", expand=True, pady=10)
        
        # Guide sections - Language-specific
        if CURRENT_LANG == "vi":
            guide_sections = [
                {
                    "title": "📋 Chuẩn Bị Trước Khi Test",
                    "steps": [
                        "1. Đảm bảo laptop đã sạc đầy pin (hoặc cắm sạc)",
                        "2. Tắt các ứng dụng không cần thiết",
                        "3. Kết nối mạng Internet (cho một số test)",
                        "4. Chuẩn bị giấy bút ghi chú (nếu cần)"
                    ]
                },
                {
                    "title": "⚙️ Hướng Dẫn Chế Độ Cơ Bản",
                    "steps": [
                        "1. Click nút '⚙️ CƠ BẢN' từ màn hình chính",
                        "2. Làm theo hướng dẫn từng bước (12 bước)",
                        "3. Mỗi bước có 3 phần:",
                        "   • 'Tại sao cần test?' - Giải thích mục đích",
                        "   • 'Cách thực hiện' - Hướng dẫn chi tiết",
                        "   • Khu vực test - Thực hiện test",
                        "4. Click 'Tiếp' để chuyển bước tiếp theo",
                        "5. Click '⏭ Bỏ qua' nếu muốn skip bước hiện tại",
                ...
                },
                {
                    ...
                    "steps": [
                        "1. Click nút '🔥 CHUYÊN GIA' từ màn hình chính",
                        "2. Bao gồm tất cả 12 bước ở chế độ Cơ bản",
                        "3. Thêm 5 bước stress test:",
                        "   • CPU Stress Test (60-180 giây)",
                        "   • Tốc độ ổ cứng (đọc/ghi)",
                        "   • GPU Stress Test (60 giây)",
                        "   • Thermal Monitor (giám sát nhiệt độ)",
                        "   • System Stability (test tổng hợp)",
                        "4. Lưu ý: Stress test có thể làm nóng máy",
                        "5. Có thể nhấn ESC để dừng stress test"
                    ]
                },
                {
                    "title": "🔧 Hướng Dẫn Kiểm Tra Riêng Lẻ",
                    "steps": [
                        "1. Click nút '🔧 RIÊNG LẺ' từ màn hình chính",
                        "2. Chọn thành phần muốn kiểm tra từ danh sách",
                        "3. Không cần làm theo thứ tự",
                        "4. Có thể test lại nhiều lần",
                        "5. Phù hợp khi chỉ muốn kiểm tra 1 thành phần cụ thể"
                    ]
                },
                {
                    "title": "📊 Cách Đọc Báo Cáo",
                    "steps": [
                        "1. Báo cáo tổng kết xuất hiện sau khi hoàn thành tất cả test",
                        "2. Các phần trong báo cáo:",
                        "   🟢 'LAPTOP TÌNH TRẠNG TỐT' - An toàn, có thể mua",
                        "   🟡 'CẦN KIỂM TRA KỸ' - Có vấn đề nhỏ, cân nhắc",
                        "   🔴 'CÓ VẤN ĐỀ NGHIÊM TRỌNG' - Không nên mua",
                        "3. Chi tiết từng bước test:",
                        "   ✅ = Tốt | ❌ = Lỗi | ⚠️ = Cảnh báo | ⏭️ = Bỏ qua",
                        "4. Đánh giá AI - Phân tích tự động",
                        "5. Công cụ bổ sung - Đề xuất tools chuyên nghiệp"
                    ]
                },
                {
                    "title": "💾 Xuất Báo Cáo",
                    "steps": [
                        "1. Sau khi xem báo cáo, cuộn xuống dưới cùng",
                        "2. Chọn định dạng xuất:",
                        "   📄 Export PDF - File PDF đầy đủ",
                        "   📊 Export Excel - Bảng tính Excel",
                        "   📋 Copy Text - Copy vào clipboard",
                        "3. File sẽ được lưu vào thư mục ứng dụng",
                        "4. Tên file: laptop_report_YYYYMMDD_HHMMSS"
                    ]
                },
                {
                    "title": "🎨 Các Tính Năng Khác",
                    "steps": [
                        "🌙/☀️ Chuyển Dark/Light Mode - Click nút góc trên bên phải",
                        "🌐 Chuyển ngôn ngữ VI/EN - Click nút cạnh Dark Mode",
                        "🏠 Về trang chủ - Click nút Home ở header",
                        "❓ Xem hướng dẫn bất kỳ lúc nào - Click '📚 HƯỚNG DẪN'"
                    ]
                },
                {
                    "title": "⚠️ Xử Lý Sự Cố",
                    "steps": [
                        ...
                        "   • Click nút 'Dừng Test' (nếu có)",
                        "   • Bỏ qua bước đó và tiếp tục",
                        "❗ Nếu ứng dụng bị lỗi:",
                        "   • Tắt và mở lại ứng dụng",
                        "   • Chạy với quyền Administrator",
                        "   • Kiểm tra phần mềm antivirus",
                        "❗ Nếu kết quả không chính xác:",
                        "   • So sánh với công cụ khác",
                        "   • Chạy lại test nhiều lần",
                        "   • Liên hệ hỗ trợ qua Hotline"
                    ]
                }
            ]
        else:  # English
            guide_sections = [
                {
                    ...
                    "steps": [
                        "1. Ensure laptop is fully charged (or plugged in)",
                        "2. Close unnecessary applications",
                        "3. Connect to Internet (for some tests)",
                        "4. Prepare pen and paper for notes (if needed)"
                    ]
                },
                {
                    "title": "⚙️ Basic Mode Guide",
                    "steps": [
                        "1. Click '⚙️ BASIC' button from home screen",
                        "2. Follow step-by-step instructions (12 steps)",
                        "3. Each step has 3 parts:",
                        "   • 'Why test?' - Explains purpose",
                        "   • 'How to perform' - Detailed instructions",
                        "   • Test area - Perform the test",
                        "4. Click 'Next' to go to next step",
                        "5. Click '⏭ Skip' if you want to skip current step",
                        "6. View summary report at the final step"
                    ]
                },
                {
                    "title": "🔥 Expert Mode Guide",
                    "steps": [
                        "1. Click '🔥 EXPERT' button from home screen",
                        "2. Includes all 12 steps from Basic mode",
                        "3. Plus 5 additional stress tests:",
                        "   • CPU Stress Test (60-180 seconds)",
                        "   • Disk Speed (read/write)",
                        "   • GPU Stress Test (60 seconds)",
                        "   • Thermal Monitor (temperature monitoring)",
                        "   • System Stability (combined test)",
                        "4. Note: Stress tests may heat up the laptop",
                        "5. Press ESC to stop stress test"
                    ]
                },
                {
                    "title": "🔧 Individual Testing Guide",
                    "steps": [
                        "1. Click '🔧 INDIVIDUAL' button from home screen",
                        "2. Select component to test from list",
                        "3. No need to follow order",
                        "4. Can test multiple times",
                        "5. Suitable when you only want to test specific component"
                    ]
                },
                {
                    "title": "📊 How to Read Reports",
                    "steps": [
                        "1. Summary report appears after completing all tests",
                        "2. Report sections:",
                        "   🟢 'LAPTOP IN GOOD CONDITION' - Safe to buy",
                        "   🟡 'NEEDS CAREFUL INSPECTION' - Minor issues, consider",
                        "   🔴 'SERIOUS ISSUES DETECTED' - Not recommended",
                        "3. Step-by-step test details:",
                        "   ✅ = Good | ❌ = Error | ⚠️ = Warning | ⏭️ = Skipped",
                        "4. AI Evaluation - Automated analysis",
                        "5. Additional Tools - Professional tool recommendations"
                    ]
                },
                {
                    "title": "💾 Export Reports",
                    "steps": [
                        "1. After viewing report, scroll to bottom",
                        "2. Choose export format:",
                        "   📄 Export PDF - Full PDF file",
                        "   📊 Export Excel - Excel spreadsheet",
                        "   📋 Copy Text - Copy to clipboard",
                        "3. File will be saved to application folder",
                        "4. Filename: laptop_report_YYYYMMDD_HHMMSS"
                    ]
                },
                {
                    "title": "🎨 Other Features",
                    "steps": [
                        "🌙/☀️ Switch Dark/Light Mode - Click button at top right corner",
                        "🌐 Switch VI/EN language - Click button next to Dark Mode",
                        "🏠 Return to home - Click Home button in header",
                        "❓ View guide anytime - Click '📚 GUIDE'"
                    ]
                },
                {
                    ...
                    "steps": [
                        "❗ If test freezes:",
                        "   • Press ESC to stop (stress test)",
                        "   • Click 'Stop Test' button (if available)",
        ...
        for section in guide_sections:
            ...
            ctk.CTkLabel(section_frame, text=section["title"], font=("Arial", 18, "bold"),
                        text_color=Theme.WARNING, anchor="w").pack(fill="x", padx=20, pady=(15, 10))
            
            for step in section["steps"]:
                ctk.CTkLabel(section_frame, text=step, font=Theme.BODY_FONT,
                            text_color=Theme.TEXT, anchor="w", justify="left", wraplength=1200).pack(fill="x", padx=30, pady=2)
            
            ctk.CTkLabel(section_frame, text="", height=5).pack()  # Spacer
...
class App(ctk.CTk):
    def __init__(self):
        ...
        contact_text = "📞 & 📱 Donate MoMo: 0976896621" if CURRENT_LANG == "en" else "📞 & 📱 Donate MoMo: 0976896621"
    ...
    def toggle_language_enhanced(self):
        ...
        # Update header labels FIRST (before any return)
        slogan_text = "Comprehensive laptop testing - Professional" if CURRENT_LANG == "en" else "Kiểm tra laptop toàn diện - Chuyên nghiệp"
        ...
        if hasattr(self, 'current_main_frame') and self.current_main_frame:
            ...
            # Check if it's IntroductionFrame or GuideFrame
        ...
        # Rebuild mode selection only if not handled above
        self.show_mode_selection()
