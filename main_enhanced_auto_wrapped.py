...
def run_gpu_stress(duration, queue):
    ...
    try:
        ...
        caption_text = t("GPU Stress Test - Nhấn ESC để thoát") if CURRENT_LANG == "vi" else "GPU Stress Test - Press ESC to exit"
...
class HardwareFingerprintStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        how_text = t("Nhấn ")Bắt đầu Test' để đọc thông tin phần cứng từ BIOS. Sau đó so sánh với thông tin quảng cáo của người bán." if CURRENT_LANG == "vi" else "Click 'Start Test' to read hardware information from BIOS. Then compare with seller's advertised specifications."
    ...
    def start_hardware_test(self):
        ...
        for idx, (item, icon) in enumerate(zip(self.info_items, icons)):
            ...
            loading_text = t("Đang tải...") if CURRENT_LANG == "vi" else "Loading..."
    ...
    def fetch_hardware_info(self):
        ...
        if platform.system() == "Windows" and wmi and pythoncom:
            ...
            try:
                ...
                try:
                    ...
                    if bios_date and len(str(bios_date)) >= 8:
                        ...
                        unknown_text = t("Không xác định") if CURRENT_LANG == "vi" else "Unknown"
                        bios_key = "Ngày BIOS" if CURRENT_LANG == "vi" else "BIOS Date"
                ...
                try:
                    ...
                    if processors and len(processors) > 0:
                        ...
                        no_cpu_text = t("Không tìm thấy CPU") if CURRENT_LANG == "vi" else "No CPU found"
                ...
                try:
                    ...
                    no_gpu_text = t("Không tìm thấy GPU") if CURRENT_LANG == "vi" else "No GPU found"
                ...
                try:
                    ...
                    no_hdd_text = t("Không tìm thấy ổ cứng") if CURRENT_LANG == "vi" else "No hard drive found"
            ...
            windows_only_text = t("Chỉ hỗ trợ Windows") if CURRENT_LANG == "vi" else "Windows only"
    ...
    def display_info(self, hw_info):
        ...
        result_text = t("Đã lấy định danh phần cứng") if CURRENT_LANG == "vi" else "Hardware fingerprint obtained"
        detail_text = t("Thông tin định danh phần cứng:") if CURRENT_LANG == "vi" else "Hardware fingerprint information:"
    ...
    def show_result_choices(self):
        ...
        continue_text = t("Định danh phần cứng đã hoàn thành. Bạn có muốn tiếp tục?") if CURRENT_LANG == "vi" else "Hardware fingerprinting completed. Do you want to continue?"
...
class LicenseCheckStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        why_text = t("Một máy tính có bản quyền Windows hợp lệ đảm bảo bạn nhận được các bản cập nhật bảo mật quan trọng và tránh các rủi ro pháp lý.") if CURRENT_LANG == "vi" else "A computer with valid Windows license ensures you receive important security updates and avoid legal risks."
        how_text = t("Nhấn ")Bắt đầu Test' để chạy lệnh kiểm tra trạng thái kích hoạt Windows. Kết quả sẽ hiển thị bên dưới." if CURRENT_LANG == "vi" else "Click 'Start Test' to run commands checking Windows activation status. Results will be displayed below."
    ...
    def check_license(self):
        no_check_text = t("Không thể kiểm tra") if CURRENT_LANG == "vi" else "Cannot check"
        error_text = t("Lỗi") if CURRENT_LANG == "vi" else "Error"
        ...
        if platform.system() == "Windows":
            try:
                ...
                if any(s in result for s in activated_strings):
                    activated_text = t("Windows được kích hoạt vĩnh viễn") if CURRENT_LANG == "vi" else "Windows permanently activated"
                    ...
                    not_activated_text = t("Windows chưa được kích hoạt") if CURRENT_LANG == "vi" else "Windows not activated"
                ...
                command_error_text = t("Lỗi khi chạy lệnh kiểm tra") if CURRENT_LANG == "vi" else "Error running check command"
                error_status = "Lỗi" if CURRENT_LANG == "vi" else "Error"
...
class SystemInfoStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        why_text = t("Bước này hiển thị thông tin cấu hình mà Windows nhận diện và tự động so sánh với thông tin từ BIOS để phát hiện sai lệch.") if CURRENT_LANG == "vi" else "This step displays configuration information that Windows recognizes and automatically compares with BIOS information to detect discrepancies."
        how_text = t("Đối chiếu thông tin dưới đây với bước trước và với thông tin quảng cáo. Nếu mọi thứ khớp, chọn ")Cấu hình khớp'." if CURRENT_LANG == "vi" else "Cross-check the information below with the previous step and with advertised specifications. If everything matches, select 'Config matches'."
        super().__init__(master, title, why_text, how_text, **kwargs)
        self.action_frame.grid_rowconfigure(0, weight=1)
        self.action_frame.grid_columnconfigure(0, weight=1)
        ...
        try:
            ...
            if platform.system() == "Windows" and wmi and pythoncom:
                ...
                try:
                    ...
                    not_found_text = t("Không tìm thấy") if CURRENT_LANG == "vi" else "Not found"
    ...
    def perform_comparison(self):
        ...
        if cpu_bios == na_text or cpu_win == na_text:
            no_compare_text = t("Kết quả: Không thể so sánh (thiếu dữ liệu)") if CURRENT_LANG == "vi" else "Result: Cannot compare (missing data)"
            ...
            match_text = t("✅ Kết quả: Khớp") if CURRENT_LANG == "vi" else "✅ Result: Match"
            result_label.configure(text=match_text, text_color=Theme.SUCCESS)
        else:
            mismatch_text = t("⚠️ Cảnh báo: Có sai lệch - Kiểm tra lại!") if CURRENT_LANG == "vi" else "⚠️ Warning: Mismatch detected - Check again!"
...
class HardDriveHealthStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        why_text = t("Ổ cứng sắp hỏng là mối rủi ro mất dữ liệu cực lớn. Bước này đọc ")báo cáo y tế' (S.M.A.R.T.) của ổ cứng để đánh giá độ bền." if CURRENT_LANG == "vi" else "A failing hard drive is an extremely high risk of data loss. This step reads the 'health report' (S.M.A.R.T.) of the hard drive to assess durability."
        how_text = t("Chú ý đến mục ")Trạng thái'. 'Tốt' là bình thường. 'Lỗi/Cảnh báo' là rủi ro cao. Bước tiếp theo sẽ kiểm tra tốc độ thực tế." if CURRENT_LANG == "vi" else "Pay attention to the 'Status' section. 'Good' is normal. 'Error/Warning' is high risk. The next step will check actual speed."
        super().__init__(master, title, why_text, how_text, **kwargs)
        self.action_frame.grid_rowconfigure(0, weight=1)
        self.action_frame.grid_columnconfigure(0, weight=1)
    ...
    def fetch_drive_info(self):
        ...
        if platform.system() == "Windows" and wmi and pythoncom:
            ...
            try:
                ...
                drives = c.Win32_DiskDrive()
                if not drives:
                    no_drive_text = t("Không tìm thấy ổ cứng") if CURRENT_LANG == "vi" else "No hard drive found"
                    error_status = "Lỗi" if CURRENT_LANG == "vi" else "Error"
                ...
                no_smart_text = t("Không thể đọc S.M.A.R.T") if CURRENT_LANG == "vi" else "Cannot read S.M.A.R.T"
                error_text = f"Lỗi: {e}" if CURRENT_LANG == "vi" else f"Error: {e}"
    ...
    def display_info(self, drives_info):
        ...
        for drive_data in drives_info:
            ...
            good_text = t("Tốt") if CURRENT_LANG == "vi" else "Good"
...
class ScreenTestStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        why_text = t("Màn hình là một trong những linh kiện đắt tiền và dễ hỏng nhất. Lỗi điểm chết, hở sáng, ám màu hay ")ung thư panel' (chớp giật ở cạnh viền) là những vấn đề nghiêm trọng." if CURRENT_LANG == "vi" else "The screen is one of the most expensive and fragile components. Dead pixels, backlight bleeding, color staining, or 'panel cancer' (flickering at edges) are serious issues."
        how_text = t("Nhấn ")Bắt đầu Test' để chạy test màn hình tự động. Test sẽ hiển thị các màu khác nhau, nhấn ESC để dừng bất cứ lúc nào." if CURRENT_LANG == "vi" else "Click 'Start Test' to run automatic screen test. The test will display different colors, press ESC to stop at any time."
        super().__init__(master, title, why_text, how_text, **kwargs)
    ...
    def start_screen_test(self):
        def run_test():
            ...
            for i, ((bg, fg), name) in enumerate(zip(colors, names)):
                ...
                esc_text = t("ESC để dừng") if CURRENT_LANG == "vi" else "ESC to stop"
...
class KeyboardTestStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        why_text = t("Một phím bị liệt, kẹt, hoặc touchpad bị loạn/mất cử chỉ đa điểm sẽ làm gián đoạn hoàn toàn công việc.") if CURRENT_LANG == "vi" else "A dead, stuck key, or malfunctioning touchpad/lost multi-touch gestures will completely disrupt work."
        how_text = t("Gõ lần lượt tất cả các phím. Phím bạn gõ sẽ sáng lên màu xanh dương, và chuyển sang xanh lá khi được nhả ra. Vẽ trên vùng test touchpad, thử click trái/phải.") if CURRENT_LANG == "vi" else "Type all keys sequentially. Keys you press will light up blue, and turn green when released. Draw on the touchpad test area, try left/right clicks."
        super().__init__(master, title, why_text, how_text, **kwargs)
        self.key_widgets = {}
    ...
    def create_touchpad_test(self):
        ...
        left_click_text = t("Click trái: 0") if CURRENT_LANG == "vi" else "Left click: 0"
        right_click_text = t("Click phải: 0") if CURRENT_LANG == "vi" else "Right click: 0"
    ...
    def on_left_click(self, event):
        ...
        left_text = t("Click trái:") if CURRENT_LANG == "vi" else "Left click:"
    ...
    def on_right_click(self, event):
        ...
        right_text = t("Click phải:") if CURRENT_LANG == "vi" else "Right click:"
...
class BaseStressTestStep(BaseStepFrame):
    ...
    def stop_test(self):
        ...
        stop_text = t("Test đã được dừng bởi người dùng.") if CURRENT_LANG == "vi" else "Test stopped by user."
    ...
    def handle_message(self, msg):
        ...
        if msg_type == 'status':
            ...
            if not self._completed:
                 ...
                 completed_text = t("Hoàn thành") if CURRENT_LANG == "vi" else "Completed"
...
class CPUStressTestStep(BaseStressTestStep):
    def __init__(self, master, **kwargs):
        ...
        why_text = t("Một CPU quá nhiệt sẽ tự giảm hiệu năng (throttling) gây giật lag. Bài test này sẽ đẩy CPU lên 100% tải để kiểm tra khả năng tản nhiệt của máy.") if CURRENT_LANG == "vi" else "An overheated CPU will automatically reduce performance (throttling) causing stuttering and lag. This test will push CPU to 100% load to check the machine's cooling capability."
        how_text = t("Nhấn ")Bắt đầu Test' trong 2-5 phút. Theo dõi biểu đồ nhiệt độ. Nếu nhiệt độ ổn định dưới 95°C và không có hiện tượng treo máy, hệ thống tản nhiệt hoạt động tốt." if CURRENT_LANG == "vi" else "Click 'Start Test' for 2-5 minutes. Monitor the temperature chart. If temperature stays stable under 95°C and no system freezing occurs, the cooling system works well."
        super().__init__(master, title, why_text, how_text, **kwargs)
    ...
    def finalize_test(self, msg):
        ...
        header_text = t("📊 Kết Quả CPU Stress Test") if CURRENT_LANG == "vi" else "📊 CPU Stress Test Results"
        ...
        if freq_locked:
            ...
            lock_text = t("🔒 PHÁT HIỆN KHÓA XUNG CPU") if CURRENT_LANG == "vi" else "🔒 CPU FREQUENCY LOCK DETECTED"
            ...
            if CURRENT_LANG == "vi":
                analysis_text = t("📊 Phân tích:")
                freq_text = f"• Tần số cố định tại {min_freq:.0f}MHz (biến thiên <5%)"
                cpu_text = t("• CPU không tự điều chỉnh tần số theo tải")
                causes_text = t("\n🔍 Nguyên nhân có thể:")
                bios_text = t("• Cài đặt BIOS khóa tần số (Fixed Frequency)")
                software_text = t("• Phần mềm quản lý nguồn hạn chế CPU")
                power_text = t("• Chế độ tiết kiệm pin cực mạnh")
            else:
        ...
        if max_temp and max_temp > 95:
            ...
            temp_warning_text = t("🚨 CẢNH BÁO: Nhiệt độ nguy hiểm! CPU có thể bị hỏng.") if CURRENT_LANG == "vi" else "🚨 WARNING: Dangerous temperature! CPU may be damaged."
        ...
        good_text = t("✓ CPU Tốt") if CURRENT_LANG == "vi" else "✓ CPU Good"
        accept_text = t("✓ Chấp Nhận") if CURRENT_LANG == "vi" else "✓ Accept"
        ...
        issue_text = t("✗ CPU Có Vấn Đề") if CURRENT_LANG == "vi" else "✗ CPU Issues"
...
class GPUStressTestStep(BaseStressTestStep):
    ...
    def finalize_test(self, msg):
        ...
        header_text = t("📊 Kết Quả GPU Stress Test") if CURRENT_LANG == "vi" else "📊 GPU Stress Test Results"
        ctk.CTkLabel(self.results_frame, text=header_text, font=Theme.SUBHEADING_FONT, text_color=Theme.ACCENT).pack(pady=10)
        ...
        gpu_stable_text = t("GPU ổn định") if CURRENT_LANG == "vi" else "GPU stable"
        ...
        gpu_issue_text = t("GPU có vấn đề") if CURRENT_LANG == "vi" else "GPU issues"
        gpu_unstable_text = t("GPU không ổn định") if CURRENT_LANG == "vi" else "GPU unstable"
...
class HardDriveSpeedStep(BaseStressTestStep):
    def __init__(self, master, **kwargs):
        ...
        why_text = t("Tốc độ đọc/ghi ảnh hưởng trực tiếp đến hiệu năng hệ thống.") if CURRENT_LANG == "vi" else "Read/write speed directly affects system performance."
        how_text = t("Nhấn ")Bắt đầu Test' để kiểm tra tốc độ ổ cứng thực tế." if CURRENT_LANG == "vi" else "Click 'Start Test' to check actual hard drive speed."
        super().__init__(master, title, why_text, how_text, **kwargs)
        self.TEST_DURATION = 60
        self.estimated_time = "1-2 phút" if CURRENT_LANG == "vi" else "1-2 minutes"
    ...
    def finalize_test(self, msg):
        ...
        slow_speed_text = t("Tốc độ chậm") if CURRENT_LANG == "vi" else "Slow speed"
...
class PhysicalInspectionStep(BaseStepFrame):
    ...
    def show_result_choices(self):
        for widget in self.result_container.winfo_children():
            widget.destroy()
        question_text = t("Tình trạng vật lý tổng thể của máy như thế nào?") if CURRENT_LANG == "vi" else "What is the overall physical condition of the laptop?"
        ctk.CTkLabel(self.result_container, text=question_text, font=Theme.SUBHEADING_FONT).pack(pady=15)
        button_bar = ctk.CTkFrame(self.result_container, fg_color="transparent")
        button_bar.pack(pady=15)
        
        btn1_text = t("Rất tốt") if CURRENT_LANG == "vi" else "Excellent"
        btn2_text = t("Tốt") if CURRENT_LANG == "vi" else "Good"
        btn3_text = t("Trung bình") if CURRENT_LANG == "vi" else "Average"
        btn4_text = t("Kém") if CURRENT_LANG == "vi" else "Poor"
...
class BIOSCheckStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        how_text = t("Xem checklist bên dưới để kiểm tra từng mục trong BIOS")
    ...
    def create_bios_checklist(self):
        ...
        for item in [
            ...
            "✓ AMD Turbo Core/Precision Boost: Phải 'Enabled'",
        ...
        for item in [
            ...
            "✓ Kiểm tra dung lượng RAM hiển thị khớp thực tế"
        ]:
        ...
        for item in [
            "✓ Battery Charge Threshold: 80% (kéo dài tuổi thọ pin)",
            "✓ Rapid Charge: 'Enabled' (sạc nhanh)",
            "✓ Always On USB: Tùy nhu cầu",
            "✓ Fn/Ctrl Key Swap: Tùy thích"
        ]:
            ctk.CTkLabel(thinkpad_frame, text=item, font=Theme.SMALL_FONT, text_color="white").pack(anchor="w", padx=25, pady=1)
    ...
    def show_result_choices(self):
        for widget in self.result_container.winfo_children():
            widget.destroy()
        question_text = t("Các cài đặt trong BIOS có chính xác và an toàn không?") if CURRENT_LANG == "vi" else "Are the BIOS settings correct and safe?"
        ctk.CTkLabel(self.result_container, text=question_text, font=Theme.SUBHEADING_FONT).pack(pady=15)
        button_bar = ctk.CTkFrame(self.result_container, fg_color="transparent")
        button_bar.pack(pady=15)
        
        btn_yes_text = t("Có, mọi cài đặt đều đúng") if CURRENT_LANG == "vi" else "Yes, all settings are correct"
        btn_no_text = t("Không, có cài đặt sai/bị khóa") if CURRENT_LANG == "vi" else "No, incorrect settings/locked"
...
class BatteryHealthStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        why_text = t("Pin là nguồn năng lượng di động của laptop. Pin hỏng hoặc chai sẽ giảm thời gian sử dụng và có thể gây nguy hiểm.") if CURRENT_LANG == "vi" else "Battery is the mobile power source of the laptop. A damaged or degraded battery will reduce usage time and may be dangerous."
        how_text = t("Thông tin pin sẽ được tự động thu thập. Kiểm tra các thông số dưới đây và đánh giá tình trạng pin.") if CURRENT_LANG == "vi" else "Battery information will be automatically collected. Check the parameters below and assess battery condition."
        super().__init__(master, title, why_text, how_text, **kwargs)
    ...
    def get_battery_info(self):
        ...
        try:
            ...
            if battery:
                ...
                analysis_text = t("📊 Phân Tích Chi Tiết") if CURRENT_LANG == "vi" else "📊 Detailed Analysis"
                ...
                if battery.secsleft != psutil.POWER_TIME_UNLIMITED and battery.secsleft > 0:
                    ...
                    if CURRENT_LANG == "vi":
                        time_text = t("Đang sạc điện") if battery.power_plugged else "Không xác định"
                    else:
    ...
    def show_result_choices(self):
        ...
        assess_text = t("Dựa trên phân tích trên, đánh giá tình trạng pin:") if CURRENT_LANG == "vi" else "Based on the analysis above, assess battery condition:"
...
class AudioTestStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        why_text = t("Hệ thống âm thanh quan trọng cho giải trí và họp trực tuyến. Loa bị rè, micro không hoạt động sẽ ảnh hưởng đến trải nghiệm multimedia.") if CURRENT_LANG == "vi" else "Audio system is important for entertainment and online meetings. Distorted speakers or non-working microphone will affect multimedia experience."
        how_text = t("Phát bài nhạc test và kiểm tra micro với biểu đồ sóng âm.") if CURRENT_LANG == "vi" else "Play test music and check microphone with audio waveform display."
        super().__init__(master, title, why_text, how_text, **kwargs)
    ...
    def stop_music(self):
        ...
        if pygame:
            try:
                ...
                pass
        stopped_music_text = t("⏹️ Đã dừng nhạc") if CURRENT_LANG == "vi" else "⏹️ Music stopped"
        self.recording_status.configure(text=stopped_music_text)
    
    def start_recording(self):
        recording_text = t("🎤 Đang ghi âm... Nói vào micro") if CURRENT_LANG == "vi" else "🎤 Recording... Speak into microphone"
        self.recording_status.configure(text=recording_text)
        
        def mock_recording():
            time.sleep(5)
            recorded_text = t("✅ Đã ghi âm 5 giây") if CURRENT_LANG == "vi" else "✅ Recorded 5 seconds"
            self.recording_status.configure(text=recorded_text)
        
        threading.Thread(target=mock_recording, daemon=True).start()
    
    def stop_recording(self):
        stop_recording_text = t("✅ Đã dừng ghi âm") if CURRENT_LANG == "vi" else "✅ Recording stopped"
...
class WebcamTestStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        why_text = t("Webcam cần thiết cho video call và họp trực tuyến. Camera không hoạt động hoặc chất lượng kém sẽ ảnh hưởng đến giao tiếp.") if CURRENT_LANG == "vi" else "Webcam is essential for video calls and online meetings. Non-working camera or poor quality will affect communication."
        how_text = t("Nhấn ")Bắt đầu Test' để mở camera. Kiểm tra chất lượng hình ảnh, độ phân giải và che camera để test phát hiện vật cản." if CURRENT_LANG == "vi" else "Click 'Start Test' to open camera. Check image quality, resolution, and cover camera to test obstruction detection."
        super().__init__(master, title, why_text, how_text, **kwargs)
    ...
    def create_webcam_test(self):
        ...
        camera_text = t("Camera chưa khởi động - Khung hình 640x480") if CURRENT_LANG == "vi" else "Camera not started - Frame 640x480"
    ...
    def stop_camera_test(self):
        ...
        stopped_text = t("Camera đã dừng") if CURRENT_LANG == "vi" else "Camera stopped"
...
class NetworkTestStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        why_text = t("Kết nối mạng ổn định quan trọng cho công việc và giải trí. Test này kiểm tra tốc độ, độ trễ và chất lượng kết nối WiFi/Ethernet.") if CURRENT_LANG == "vi" else "Stable network connection is important for work and entertainment. This test checks speed, latency and WiFi/Ethernet connection quality."
        how_text = t("Nhấn ")Bắt đầu Test' để kiểm tra kết nối Internet, DNS, tốc độ mạng và thông tin WiFi." if CURRENT_LANG == "vi" else "Click 'Start Test' to check Internet connection, DNS, network speed and WiFi information."
        super().__init__(master, title, why_text, how_text, **kwargs)
        self.test_results = {}
        self.is_testing = False
...
class ThermalMonitorStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        why_text = t("Giám sát nhiệt độ và hiệu năng real-time giúp phát hiện vấn đề tản nhiệt, throttling và hiệu năng không ổn định.") if CURRENT_LANG == "vi" else "Real-time temperature and performance monitoring helps detect cooling issues, throttling and unstable performance."
        how_text = t("Nhấn ")Start Monitor' để bắt đầu giám sát. Có thể chạy Stress Test để kiểm tra khả năng tản nhiệt dưới tải cao." if CURRENT_LANG == "vi" else "Click 'Start Monitor' to begin monitoring. Can run Stress Test to check cooling capability under high load."
        super().__init__(master, title, why_text, how_text, **kwargs)
...
class SystemStabilityStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        why_text = t("Test tổng hợp CPU+GPU+RAM để đảm bảo hệ thống ổn định dưới tải nặng kéo dài.") if CURRENT_LANG == "vi" else "Combined CPU+GPU+RAM test to ensure system stability under prolonged heavy load."
        how_text = t("Nhấn ")Bắt đầu Test' để chạy test kết hợp 3-5 phút. Quan sát nhiệt độ và hiệu năng." if CURRENT_LANG == "vi" else "Click 'Start Test' to run combined test for 3-5 minutes. Monitor temperature and performance."
        super().__init__(master, title, why_text, how_text, **kwargs)
        self.is_testing = False
...
class SummaryStep(BaseStepFrame):
    ...
    def create_simple_summary(self, results):
        ...
        ai_title = "🤖 Đánh giá của AI" if CURRENT_LANG == "vi" else "🤖 AI Assessment"
        ctk.CTkLabel(ai_frame, text=ai_title, font=Theme.HEADING_FONT, text_color="white").pack(pady=(15, 5))
        
        # AI Disclaimer
        disclaimer_text = t("⚠️ LƯU Ý: Ứng dụng được phát triển dựa trên AI nên có thể có sai sót. Khuyến khích kiểm tra thêm bằng các ứng dụng chuyên dụng được cung cấp bên dưới.") if CURRENT_LANG == "vi" else "⚠️ DISCLAIMER: This AI-developed application may contain errors. We recommend additional verification using the professional tools provided below."
        ...
        if not results:
            no_results_text = t("Chưa có kết quả test nào") if CURRENT_LANG == "vi" else "No test results available"
        ...
        tools_desc_text = t("Để xác minh kết quả AI và kiểm tra sâu hơn, hãy sử dụng các công cụ chuyên nghiệp sau:") if CURRENT_LANG == "vi" else "To verify AI results and perform deeper inspection, use these professional tools:"
        ctk.CTkLabel(tools_frame, text=tools_desc_text, font=Theme.BODY_FONT, text_color=Theme.TEXT_SECONDARY).pack(padx=20)
        
        verification_text = t("💡 Khuyến khích so sánh kết quả với các tools này để đảm bảo độ chính xác cao nhất.") if CURRENT_LANG == "vi" else "💡 Recommended to compare results with these tools for highest accuracy."
    ...
    def copy_report(self):
        import tkinter as tk
        # Create summary text
        summary_text = t("=== BÁO CÁO KIỂM TRA LAPTOP ===\n\n")
...
class WizardFrame(ctk.CTkFrame):
    ...
    def create_header(self):
        ...
        home_text = t("🏠 Trang chủ") if CURRENT_LANG == "vi" else "🏠 Home"
    ...
    def _get_steps_for_mode(self, mode):
        basic_steps = [
            ...
            ("Kiểm Tra Ngoại Hình" if CURRENT_LANG == "vi" else "Physical Inspection", PhysicalInspectionStep),
            ("Kiểm Tra BIOS" if CURRENT_LANG == "vi" else "BIOS Check", BIOSCheckStep),
    ...
    def confirm_exit(self):
        ...
        title_text = t("Hoàn thành") if CURRENT_LANG == "vi" else "Completed"
...
class IndividualTestFrame(ctk.CTkFrame):
    def __init__(self, master, icon_manager, app=None):
        ...
        title_text = t("🔧 Chọn Bài Test Riêng Lẻ") if CURRENT_LANG == "vi" else "🔧 Select Individual Test"
        ctk.CTkLabel(header, text=title_text, font=Theme.HEADING_FONT, text_color=Theme.ACCENT).pack(pady=15)
        
        back_btn_text = t("← Quay lại") if CURRENT_LANG == "vi" else "← Back"
        ...
        for test_name, test_class, icon, desc in all_tests:
            ...
            run_text = t("▶️ Chạy Test") if CURRENT_LANG == "vi" else "▶️ Run Test"
    ...
    def run_single_test(self, test_class, test_name):
        ...
        close_text = t("✖ Đóng") if CURRENT_LANG == "vi" else "✖ Close"
...
class ModeSelectionFrame(ctk.CTkFrame):
    def __init__(self, master, mode_callback, icon_manager, app=None):
        ...
        mode_text = t("🎯 Chọn chế độ kiểm tra phù hợp:") if CURRENT_LANG == "vi" else "🎯 Choose appropriate testing mode:"
...
class App(ctk.CTk):
    def __init__(self):
        ...
        slogan_text = t("Kiểm tra laptop toàn diện - Chuyên nghiệp") if CURRENT_LANG == "vi" else "Comprehensive Laptop Testing - Professional"
        ...
        dev_text = t("💻 Phát triển bởi: Laptop Lê Ẩn & Gemini AI") if CURRENT_LANG == "vi" else "💻 Developed by: Laptop Lê Ẩn & Gemini AI"
        ...
        address_text = t("📍 237/1C Tôn Thất Thuyết, P. Vĩnh Hội, (P.3, Q.4 cũ), TPHCM") if CURRENT_LANG == "vi" else "📍 237/1C Ton That Thuyet St., Vinh Hoi Ward, (Old Ward 3, Dist. 4), HCMC"
    ...
    def toggle_language_enhanced(self):
        ...
        if CURRENT_LANG == "en":
            self.lang_btn.configure(text=t("🇻🇳 Tiếng Việt"))
            self.slogan_label.configure(text="Comprehensive Laptop Testing - Professional")
            self.dev_label.configure(text=t("💻 Developed by: Laptop Lê Ẩn & Gemini AI"))
        else:
            self.lang_btn.configure(text="🇺🇸 English")
            self.slogan_label.configure(text=t("Kiểm tra laptop toàn diện - Chuyên nghiệp"))
            self.dev_label.configure(text=t("💻 Phát triển bởi: Laptop Lê Ẩn & Gemini AI"))
