    ...
    def __init__(self):
        ...
        self.translations = {
            "vi": {
                ...
                "select_mode": "Select Test Mode", "basic_mode": "Chế Độ Cơ Bản", "expert_mode": "Expert Mode",
...
class HardwareFingerprintStep(BaseStepFrame):
        ...
        if platform.system() == "Windows":
            ...
            try:
                ...
                try:
                    ...
                    hw_info["Hard Drive Model"] = "Lỗi đọc ổ cứng"
    ...
    def display_info(self, hw_info):
        ...
        self.mark_completed({"Result": "Hardware identification retrieved", "Status": "Good", "Details": f"Thông tin định danh phần cứng:\n{full_details}"}, auto_advance=False)
        ...
        if platform.system() == "Windows":
            try:
                ...
                if any(s in result for s in activated_strings):
                    status, color, result_data = "Windows được kích hoạt vĩnh viễn", theme_manager.get_color('SUCCESS'), {"Result": "Permanently activated", "Status": "Good"}
                    ...
                    status, color, result_data = f"Windows sẽ hết hạn vào {expiry_date}", theme_manager.get_color('WARNING'), {"Result": f"Sẽ hết hạn ({expiry_date})", "Status": "Error"}
                else: status, color, result_data = "Windows chưa được kích hoạt", theme_manager.get_color('ERROR'), {"Result": "Not activated", "Status": "Error"}
            except (subprocess.CalledProcessError, FileNotFoundError):
                status, color, result_data = "Lỗi khi chạy lệnh kiểm tra", theme_manager.get_color('ERROR'), {"Result": "Lỗi khi chạy lệnh kiểm tra", "Status": "Error"}
        else:
...
class SystemInfoStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, "Windows Configuration", "Thông tin hệ thống", "Tự động đọc", **kwargs)
...
class HardDriveHealthStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, "Hard Drive Health", "Kiểm tra S.M.A.R.T", "Tự động kiểm tra", **kwargs)
        self.mark_completed({"Result": "OK", "Status": "Good"}, auto_advance=True)

# Bước 7: Screen Test - placeholder
class ScreenTestStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, "Display", "Test màn hình", "Kiểm tra pixel", **kwargs)
        self.mark_completed({"Result": "OK", "Status": "Good"}, auto_advance=True)

# Bước 8: Keyboard & Touchpad - FULL IMPLEMENTATION
class KeyboardVisualTestStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, "Keyboard & Touchpad & Mouse", "A dead, stuck key, or malfunctioning touchpad/multi-touch gestures will completely disrupt work.", "**Bàn phím:** Gõ lần lượt tất cả các phím. Phím bạn gõ sẽ sáng lên màu xanh dương, và chuyển sang xanh lá khi được nhả ra.\n**Touchpad & Chuột:**\n   1. Dùng 1 ngón tay vẽ lên vùng màu xám để kiểm tra điểm chết cảm ứng.\n   2. Click trái/phải để test nút bấm.\n   3. Dùng 2 ngón tay để cuộn lên/xuống.", **kwargs)
...
class PortsConnectivityStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, "Ports & Connectivity", "Test cổng", "Cắm thiết bị", **kwargs)
        self.mark_completed({"Result": "OK", "Status": "Good"}, auto_advance=True)

# Bước 10: Battery - placeholder
class BatteryHealthStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, "Battery", "Kiểm tra pin", "Đọc thông tin pin", **kwargs)
        self.mark_completed({"Result": "OK", "Status": "Good"}, auto_advance=True)
...
class HardDriveSpeedStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, "Hard Drive Speed", "Test tốc độ", "Benchmark", **kwargs)
        self.mark_completed({"Result": "OK", "Status": "Good"}, auto_advance=True)
...
class SummaryStep(BaseStepFrame):
    ...
    def _create_result_item(self, step_name, result_data):
        status = result_data.get("Status", "Không rõ")
    ...
    def create_header(self):
        ...
        def show_step(self, step_index):
            ...
            if step_index < total_steps:
                ...
                self.step_label.configure(text=f"Summary ({total_steps} bước hoàn thành)")
