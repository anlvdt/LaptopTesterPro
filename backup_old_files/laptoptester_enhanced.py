...
# Language dictionary with enhanced content
LANG = {
    "vi": {
...
# Enhanced BaseStepFrame with real functionality
...
class SystemInfoStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        info_frame = ctk.CTkFrame(self.action_frame, fg_color=Theme.get_card())
        ...
        for key, value in system_info.items():
            ...
            ctk.CTkLabel(row, text=f"{key}:", font=Theme.BODY_FONT, anchor="w").pack(side="left")
            ctk.CTkLabel(row, text=value, font=Theme.BODY_FONT, anchor="e").pack(side="right")
        
        ctk.CTkButton(self.action_frame, text="Hoàn thành", height=Theme.BUTTON_HEIGHT,
                     command=lambda: self.mark_completed(system_info, True)).pack(pady=20)
...
class HardDriveHealthStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        drives_frame = ctk.CTkFrame(self.action_frame, fg_color=Theme.get_card())
        drives_frame.pack(fill="x", padx=20, pady=10)
        ...
        for partition in psutil.disk_partitions():
            try:
                ...
                drive_text = f"{partition.device} - {used_gb}/{total_gb} GB ({percent:.1f}%)"
                color = Theme.ERROR if percent > 90 else Theme.SUCCESS if percent < 70 else Theme.WARNING
                
                ctk.CTkLabel(drives_frame, text=drive_text, font=Theme.BODY_FONT, text_color=color).pack(pady=5)
                drives_info[partition.device] = {"usage": percent, "status": "OK" if percent < 90 else "Warning"}
            except:
                continue
        
        ctk.CTkButton(self.action_frame, text="Hoàn thành", height=Theme.BUTTON_HEIGHT,
                     command=lambda: self.mark_completed(drives_info, True)).pack(pady=20)
    ...
    def __init__(self, master, **kwargs):
        ...
        try:
            ...
            if battery:
                ...
                color = Theme.ERROR if percent < 20 else Theme.WARNING if percent < 50 else Theme.SUCCESS
                
                ctk.CTkLabel(self.action_frame, text=f"Mức pin: {percent}%", 
                           font=Theme.BODY_FONT, text_color=color).pack(pady=10)
                ctk.CTkLabel(self.action_frame, text=f"Trạng thái: {plugged}", 
                           font=Theme.BODY_FONT).pack(pady=5)
                
                result = {"percent": percent, "plugged": battery.power_plugged}
            else:
                ctk.CTkLabel(self.action_frame, text="Không phát hiện pin (máy bàn)", 
                           font=Theme.BODY_FONT, text_color=Theme.WARNING).pack(pady=10)
                result = {"type": "desktop"}
        except:
            ctk.CTkLabel(self.action_frame, text="Lỗi kiểm tra pin", 
                       font=Theme.BODY_FONT, text_color=Theme.ERROR).pack(pady=10)
            result = {"error": "check_failed"}
        
        ctk.CTkButton(self.action_frame, text="Hoàn thành", height=Theme.BUTTON_HEIGHT,
                     command=lambda: self.mark_completed(result, True)).pack(pady=20)
...
class NetworkTestStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        try:
            ...
            ctk.CTkLabel(self.action_frame, text=f"Hostname: {hostname}", font=Theme.BODY_FONT).pack(pady=5)
            ctk.CTkLabel(self.action_frame, text=f"IP: {local_ip}", font=Theme.BODY_FONT).pack(pady=5)
            result = {"hostname": hostname, "ip": local_ip}
        except:
            ctk.CTkLabel(self.action_frame, text="Không thể lấy thông tin mạng", font=Theme.BODY_FONT, text_color=Theme.ERROR).pack(pady=5)
            result = {"error": "network_failed"}
        
        ctk.CTkButton(self.action_frame, text="Hoàn thành", height=Theme.BUTTON_HEIGHT,
                     command=lambda: self.mark_completed(result, True)).pack(pady=10)
...
class PortsConnectivityStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        info_text = "Cắm thiết bị vào các cổng USB, HDMI, Audio để kiểm tra"
        ctk.CTkLabel(self.action_frame, text=info_text, font=Theme.BODY_FONT).pack(pady=10)
...
class NetworkTestStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        try:
            ...
            ctk.CTkLabel(self.action_frame, text="Không thể lấy thông tin mạng", font=Theme.BODY_FONT).pack(pady=5)
...
class CPUStressTestStep(BaseStepFrame):
    ...
    def create_ui(self):
        ...
        ctk.CTkLabel(self.action_frame, text=cpu_info, font=Theme.BODY_FONT).pack(pady=10)
        
        self.cpu_label = ctk.CTkLabel(self.action_frame, text=f"CPU Usage: {psutil.cpu_percent()}%", font=Theme.BODY_FONT)
        self.cpu_label.pack(pady=10)
        
        ctk.CTkButton(self.action_frame, text="Bắt đầu kiểm tra", height=Theme.BUTTON_HEIGHT,
                     command=self.start_cpu_test, fg_color=Theme.ACCENT).pack(pady=20)
        
        ctk.CTkButton(self.action_frame, text="Hoàn thành", height=Theme.BUTTON_HEIGHT,
                     command=lambda: self.mark_completed({"cpu_cores": psutil.cpu_count()}, True)).pack(pady=20)
    ...
    def start_cpu_test(self):
        ...
        self.cpu_label.configure(text="Đang chạy stress test...", text_color=Theme.WARNING)
        threading.Thread(target=cpu_stress, daemon=True).start()
        
        def update_cpu():
            self.cpu_label.configure(text=f"CPU Usage: {psutil.cpu_percent()}%", text_color=Theme.SUCCESS)
        
        self.after(11000, update_cpu)
...
class HardDriveSpeedStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        info_text = "Benchmark tốc độ đọc/ghi của ổ cứng"
        ctk.CTkLabel(self.action_frame, text=info_text, font=Theme.BODY_FONT).pack(pady=10)
...
class GPUStressTestStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        info_text = "Stress test card đồ họa và kiểm tra hiệu năng"
        ctk.CTkLabel(self.action_frame, text=info_text, font=Theme.BODY_FONT).pack(pady=10)
...
class ThermalPerformanceStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        try:
            ...
            if temps:
                temp_text = "Nhiệt độ hệ thống: "
                for name, entries in temps.items():
                    for entry in entries:
                        temp_text += f"{entry.current}°C "
                        break
                    break
            else:
                temp_text = "Không thể đọc nhiệt độ"
        except:
            temp_text = "Không hỗ trợ cảm biến nhiệt độ"
        
        ctk.CTkLabel(self.action_frame, text=temp_text, font=Theme.BODY_FONT).pack(pady=10)
...
class UserGuideFrame(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(header, text="📖 " + get_text("user_guide"), 
    ...
    def get_guide_content(self):
        if CURRENT_LANG == "vi":
            return [
                {"title": "🚀 Khởi động", "content": "Chạy với quyền Administrator để có kết quả chính xác nhất"},
                {"title": "🎯 Chế độ cơ bản", "content": "9 bước kiểm tra thiết yếu, phù hợp kiểm tra nhanh"},
                {"title": "⚡ Chế độ chuyên gia", "content": "13 bước kiểm tra đầy đủ với stress test"},
                {"title": "💻 Thông tin hệ thống", "content": "Hiển thị OS, CPU, RAM, kiến trúc hệ thống"},
                {"title": "🔑 Kiểm tra bản quyền", "content": "Xác định Windows đã kích hoạt hay chưa"},
                {"title": "💾 Kiểm tra ổ cứng", "content": "Dung lượng và % sử dụng các ổ đĩa"},
                {"title": "🖥️ Kiểm tra màn hình", "content": "Độ phân giải và test 6 màu cơ bản"},
                {"title": "⌨️ Kiểm tra bàn phím", "content": "Nhấn phím để test, theo dõi phím đã nhấn"},
                {"title": "🔋 Kiểm tra pin", "content": "Mức pin, trạng thái sạc, phát hiện máy bàn"},
                {"title": "🔊 Kiểm tra âm thanh", "content": "Phát âm test 1000Hz để kiểm tra loa"},
                {"title": "📷 Kiểm tra camera", "content": "Mở camera 2 giây để test webcam"},
                {"title": "🧠 Kiểm tra CPU", "content": "Stress test 10 giây, monitor usage"},
                {"title": "🚨 Xử lý lỗi", "content": "Chạy với admin, cài OpenCV, kiểm tra Device Manager"}
            ]
        else:
            return [
                {"title": "🚀 Getting Started", "content": "Run with Administrator rights for most accurate results"},
                {"title": "🎯 Basic Mode", "content": "9 essential test steps, suitable for quick checks"},
                {"title": "⚡ Expert Mode", "content": "13 comprehensive test steps with stress testing"},
                {"title": "💻 System Information", "content": "Display OS, CPU, RAM, system architecture"},
                {"title": "🔑 License Check", "content": "Determine if Windows is activated"},
                {"title": "💾 Storage Test", "content": "Capacity and usage percentage of drives"},
                {"title": "🖥️ Display Test", "content": "Resolution and 6 basic color test"},
                {"title": "⌨️ Keyboard Test", "content": "Press keys to test, track pressed keys"},
                {"title": "🔋 Battery Test", "content": "Battery level, charging status, detect desktop"},
                {"title": "🔊 Audio Test", "content": "Play 1000Hz test tone for speaker check"},
                {"title": "📷 Camera Test", "content": "Open camera for 2 seconds to test webcam"},
                {"title": "🧠 CPU Test", "content": "10-second stress test, monitor usage"},
                {"title": "🚨 Troubleshooting", "content": "Run as admin, install OpenCV, check Device Manager"}
            ]
...
# Enhanced frames with user guide integration
class OverviewFrame(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        ctk.CTkButton(actions, text="📖 " + get_text("user_guide"), height=80, font=Theme.SUBHEADING_FONT,
...
# Enhanced ModeSelectionFrame with user guide tab
class ModeSelectionFrame(ctk.CTkFrame):
    def __init__(self, master, mode_callback, icon_manager):
        ...
        # Enhanced tabview with improved styling
...
# Rest of the classes remain the same (WizardFrame, SummaryStep, etc.)
...
if __name__ == "__main__":
    if not is_admin():
        print("Yêu cầu quyền Administrator để chạy chính xác...")
        if not run_as_admin():
            print("Không thể khởi động với quyền admin")
