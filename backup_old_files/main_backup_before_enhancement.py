...
class BaseStepFrame(ctk.CTkFrame):
    ...
    def mark_completed(self, result_data, auto_advance=False):
        ...
        # Auto scroll to bottom to show results - multiple methods
...
class LicenseCheckStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        how_text = "Ứng dụng sẽ tự động chạy lệnh kiểm tra trạng thái kích hoạt của Windows. Kết quả sẽ hiển thị bên dưới." if CURRENT_LANG == "vi" else "The application will automatically run commands to check Windows activation status. Results will be displayed below."
        super().__init__(master, title, why_text, how_text, **kwargs)
        self.action_frame.grid_rowconfigure(0, weight=1)
        self.action_frame.grid_columnconfigure(0, weight=1)
        
        self.status_label = ctk.CTkLabel(self.action_frame, text=get_text("checking"), font=Theme.SUBHEADING_FONT, text_color=Theme.TEXT_SECONDARY)
...
class HardDriveSpeedStep(BaseStepFrame):
    def __init__(self, master, **kwargs):
        ...
        start_text = f"{get_text('start_test_btn')} (1-2 phút)" if CURRENT_LANG == "vi" else f"{get_text('start_test_btn')} (1-2 minutes)"
        self.start_button = ctk.CTkButton(self.controls_frame, text=start_text, command=self.start_test, height=40, font=Theme.BODY_FONT)
        self.start_button.pack(side="left", padx=(0, 10))
        
        self.stop_button = ctk.CTkButton(self.controls_frame, text=get_text("stop_test_btn"), command=self.stop_test, height=40, font=Theme.BODY_FONT, state="disabled", fg_color=Theme.WARNING, text_color=Theme.TEXT)
        self.stop_button.pack(side="left")
...
class SummaryStep(BaseStepFrame):
    ...
    def create_simple_summary(self, results):
        ...
        ctk.CTkLabel(title_frame, text=get_text("report_title"), font=Theme.HEADING_FONT, text_color="white").pack(anchor="w")
        ctk.CTkLabel(title_frame, text=get_text("report_subtitle"), font=Theme.BODY_FONT, text_color="white").pack(anchor="w")
        ...
        ctk.CTkLabel(assessment_frame, text=get_text(recommendation), font=Theme.BODY_FONT, text_color="white").pack(pady=(0, 15))
    ...
    def generate_ai_assessment(self, results, success_rate):
        """Generate AI assessment based on test results"""
        if CURRENT_LANG == "vi":
            if success_rate >= 90:
                return "Laptop này ở tình trạng rất tốt. Tất cả các thành phần chính hoạt động bình thường. Đây là một lựa chọn đáng tin cậy cho việc mua laptop cũ. Giá cả có thể thương lượng ở mức cao hơn."
            elif success_rate >= 70:
                return "Laptop có một số vấn đề nhỏ nhưng vẫn có thể sử dụng được. Hãy kiểm tra kỹ các lỗi đã phát hiện và thương lượng giá cả phù hợp. Có thể cần sửa chữa một số thành phần."
            else:
                return "Laptop có nhiều vấn đề nghiêm trọng. Không khuyến nghị mua trừ khi giá rất thấp và bạn sẵn sàng đầu tư tiền sửa chữa. Các lỗi phát hiện cần được xử lý ngay."
        else:
            if success_rate >= 90:
                return "This laptop is in excellent condition. All major components are functioning normally. This is a reliable choice for buying a used laptop. The price can be negotiated at a higher level."
            elif success_rate >= 70:
                return "The laptop has some minor issues but is still usable. Please carefully check the detected errors and negotiate an appropriate price. Some components may need repair."
            else:
                return "The laptop has many serious issues. Not recommended for purchase unless the price is very low and you're willing to invest in repairs. The detected errors need immediate attention."
